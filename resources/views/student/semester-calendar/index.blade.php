<x-app-layout>
    ...
    <div class="mb-6">
        <h2 class="text-xl font-bold text-primary-dark dark:text-white">
            {{ $activeYear ? $activeYear->year . ' - ' . ucfirst(__($activeYear->semester)) : __('Academic Calendar') }}
        </h2>
        <p class="text-sm text-primary-secondary dark:text-gray-400">{{ __('Academic activities and important dates for this semester') }}</p>
    </div>
    ...
    <div class="grid gap-6">
        ...
        <div class="card-saas p-0 overflow-hidden dark:bg-gray-800 border-l-4 {{ $week ? 'border-primary-primary' : 'border-gray-400' }}">
            <div class="bg-gray-50 dark:bg-gray-900/50 px-4 py-2 border-b border-primary-light dark:border-gray-700 flex justify-between items-center">
                ...
                <div class="flex gap-4">
                    ...
                    <div class="flex-grow">
                        <div class="flex items-center gap-2 mb-1">
                            <h4 class="text-sm font-bold text-primary-dark dark:text-white">{{ $event->title }}</h4>
                            @php
                            $typeColors = [
                        ...
                        <p class="text-xs text-primary-secondary leading-relaxed">{{ $event->description }}</p>
                        @endif
                    </div>
                </div>
                @if(!$loop->last)
                <hr class="border-primary-light dark:border-gray-700">
                @endif
                @endforeach
            </div>
        </div>
        @empty
        <div class="card-saas p-12 text-center text-primary-secondary dark:bg-gray-800">
