        ...
        <div class="lg:col-span-1 flex flex-col gap-6">
            ...
            <div class="card-saas p-6">
                <div class="text-center mb-6">
                    <div
                        class="w-24 h-24 mx-auto rounded-xl bg-siakad-primary flex items-center justify-center text-white text-4xl font-bold mb-4">
                ...
                <div class="space-y-3 pt-4 border-t border-siakad-light">
                    ...
                    <div class="flex items-center gap-3 text-sm">
                        ...
                        <span class="text-siakad-dark">{{ $student->studyProgram->faculty->name ?? '-' }}</span>
                    </div>
                    <div class="flex items-center gap-3 text-sm">
                        <svg class="w-5 h-5 text-siakad-secondary" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                        ...
                        <span class="text-siakad-dark">{{ $student->studyProgram->name ?? '-' }}</span>
                    </div>
                    <div class="flex items-center gap-3 text-sm">
                        <svg class="w-5 h-5 text-siakad-secondary" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                        ...
                        <span class="text-siakad-dark">{{ __('Academic Advisor') }}:
            ...
            <div class="card-saas p-6 flex-1 flex flex-col">
                <h3 class="font-semibold text-siakad-dark mb-4">{{ __('Academic Information') }}</h3>
                <div class="grid grid-cols-2 gap-3 flex-1">
                    <div class="flex flex-col items-center justify-center p-4 bg-siakad-primary/10 rounded-xl">
                        <p class="text-2xl font-bold text-siakad-primary">{{ $student->batch }}</p>
                        <p class="text-xs text-siakad-secondary mt-1">{{ __('Batch') }}</p>
                    </div>
                    <div class="flex flex-col items-center justify-center p-4 bg-siakad-primary/10 rounded-xl">
                        @php
                        $currentYear = date('Y');
                        ...
                        <p class="text-xs text-siakad-secondary mt-1">{{ __('Semester') }}</p>
                    </div>
                    <div class="flex flex-col items-center justify-center p-4 bg-[#456882]/10 rounded-xl">
                        ...
                        <p class="text-xs text-siakad-secondary mt-1">{{ __('Total Registered Courses') }} </p>
                    </div>
                    <div class="flex flex-col items-center justify-center p-4 bg-emerald-500/10 rounded-xl">
                        ...
                        <p class="text-xs text-siakad-secondary mt-1">{{ __('Status') }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="card-saas">
                <div class="px-6 py-4 border-b border-siakad-light">
                    <h3 class="font-semibold text-siakad-dark">{{ __('Account Information') }}</h3>
                    <p class="text-xs text-siakad-secondary mt-1">{{ __('Update your account information') }}</p>
                </div>
                <form action="{{ route('students.profile.update') }}" method="POST">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-siakad-dark mb-2">{{ __('Full Name') }}</label>
                                <input type="text" name="name" value="{{ old('name', $user->name) }}"
                                    class="input-saas w-full px-4 py-2.5 text-sm"
                                    style="background-color: var(--bg-card);" required>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-siakad-dark mb-2">{{ __('Email Address') }}</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-siakad-dark mb-2">{{ __('Student ID') }}</label>
                                <input type="text" value="{{ $student->student_number }}"
                                    class="input-saas w-full px-4 py-2.5 text-sm"
                                    style="background-color: var(--bg-card); opacity: 0.6;" readonly disabled>
                                <p class="text-xs text-siakad-secondary mt-1">{{ __('Student ID cannot be changed') }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-siakad-dark mb-2">{{ __('Study Programs') }}</label>
            ...
            <div class="card-saas">
                <div class="px-6 py-4 border-b border-siakad-light">
                    <h3 class="font-semibold text-siakad-dark">{{ __('Change Password') }}</h3>
                    <p class="text-xs text-siakad-secondary mt-1">{{ __('Ensure you use a strong password') }}</p>
                </div>
                <form action="{{ route('students.profile.password') }}" method="POST">
                    ...
                    <div class="p-6 space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-siakad-dark mb-2">{{ __('Current Password') }}</label>
                            <input type="password" name="current_password" class="input-saas w-full px-4 py-2.5 text-sm"
                                style="background-color: var(--bg-card);" required>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-siakad-dark mb-2">{{ __('New Password') }}</label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-siakad-dark mb-2">{{ __('Confirm Password') }}</label>
