            ...
            <div class="flex-shrink-0 px-4 pb-6 pt-4">
                <div class="max-w-3xl mx-auto">
                    ...
                    <div class="flex items-center justify-center mt-3 text-[11px] text-slate-400 dark:text-slate-500">
                        <span>{{ __('Enter to send') }} &nbsp;•&nbsp; {{ __('Shift+Enter for new line') }} &nbsp;•&nbsp; {{ __('Powered by AI') }}</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            return {
                ...
                startThinking() {
                    const statuses = [
                        ...
                        '{{ __("Loading academic data...") }}',
                        '{{ __("Checking curriculum...") }}',
                        '{{ __("Analyzing progress...") }}',
                ...
                async sendMessage() {
                    ...
                    try {
                        const response = await fetch('{{ route("students.ai-advisor.chat") }}', {
                            method: 'POST',
                            headers: {
                        ...
                        this.messages.push({
                            role: 'assistant',
                            content: data.success ? data.message : '{{ __("Sorry, an error occurred") }}: ' + data.message
                        });
                    } catch (error) {
                        this.messages.push({
                            role: 'assistant',
                            content: '{{ __("Sorry, a network error occurred. Please try again.") }}'
                        });
                    } finally {
                        this.stopThinking();
