<x-app-layout>
    <div class="mb-10">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
                <h1 class="text-3xl font-black tracking-tight text-primary-900 dark:text-white">{{ __('Academic Situation') }}</h1>
            ...
            <div>
                <button id="generateRecommendationsBtn" class="btn-primary-saas px-6 py-3 rounded-2xl text-sm font-black flex items-center gap-2 shadow-lg shadow-primary-600/20">
    ...
    <div id="recommendationsSection" class="card-saas mb-10 p-8 hidden">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h3 class="text-base font-black text-primary-900 dark:text-white">{{ __('AI Recommendations') }}</h3>
        </div>
    ...
    <div id="loadingRecommendations" class="card-saas mb-10 p-8 hidden">
        <div class="flex items-center gap-3">
            ...
            <p class="text-sm font-black text-primary-700 dark:text-primary-300">{{ __('Generating recommendations...') }}</p>
        </div>
    </div>

    <!-- Situation Report Card -->
    <div class="card-saas mb-10 relative overflow-hidden group">
        <div class="absolute top-0 right-0 w-40 h-40 bg-gradient-to-br from-primary-500/10 to-purple-500/10 rounded-full -mr-20 -mt-20 blur-3xl transition-all duration-500 group-hover:scale-125"></div>
        <div class="relative p-8">
            <div class="flex items-center justify-between mb-8">
                <div>
                    <span class="text-xs font-black uppercase tracking-widest text-primary-400">{{ __('Your Situation') }}</span>
                    <h2 class="text-2xl font-black text-primary-900 dark:text-white mt-1">{{ $reportSummary['status_text'] }}</h2>
                </div>
                @php
                $statusColors = [
                'excellent' => 'bg-gradient-to-r from-emerald-500 to-emerald-600',
                'good' => 'bg-gradient-to-r from-primary-500 to-primary-600',
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-6">
                <div class="bg-primary-50/50 dark:bg-primary-900/20 p-4 rounded-xl border border-primary-100/50 dark:border-primary-800/50">
                    <p class="text-xs text-primary-400 font-bold uppercase tracking-widest mb-1">{{ __('GPA') }}</p>
                    <p class="text-2xl font-black text-primary-900 dark:text-white">{{ number_format($reportSummary['gpa'], 2) }}</p>
                </div>
                <div class="bg-primary-50/50 dark:bg-primary-900/20 p-4 rounded-xl border border-primary-100/50 dark:border-primary-800/50">
                    <p class="text-xs text-primary-400 font-bold uppercase tracking-widest mb-1">{{ __('Credits') }}</p>
                    <p class="text-2xl font-black text-primary-900 dark:text-white">{{ $reportSummary['total_credits'] }}</p>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
        ...
        <div class="card-saas p-8">
            <div class="flex items-center gap-3 mb-6">
                ...
                <h3 class="text-base font-black text-primary-900 dark:text-white">{{ __('Overall Attendance') }}</h3>
            </div>
            <div class="text-center">
                <p class="text-5xl font-black text-primary-900 dark:text-white">{{ $reportSummary['attendance_rate'] }}%</p>
                <div class="w-full bg-primary-50 dark:bg-primary-900/30 rounded-full h-4 mt-4 overflow-hidden">
                    <div class="h-full bg-gradient-to-r from-primary-500 to-primary-600 rounded-full" style="width: {{ $reportSummary['attendance_rate'] }}%"></div>
                </div>
            </div>
        </div>
        ...
        <div class="card-saas p-8">
            <div class="flex items-center gap-3 mb-6">
                ...
                <h3 class="text-base font-black text-primary-900 dark:text-white">{{ __('Assignment Submission') }}</h3>
            </div>
            <div class="text-center">
                <p class="text-5xl font-black text-primary-900 dark:text-white">{{ $reportSummary['submission_rate'] }}%</p>
                <div class="w-full bg-primary-50 dark:bg-primary-900/30 rounded-full h-4 mt-4 overflow-hidden">
    ...
    <div class="card-saas p-8 mb-10">
        <div class="flex items-center gap-3 mb-8">
            ...
            <div>
                <h3 class="text-base font-black text-primary-900 dark:text-white">{{ __('Current Semester Classes') }}</h3>
                <p class="text-sm text-primary-400 mt-1">{{ $activeAcademicYear ? $activeAcademicYear->year . ' ' . ucfirst(__($activeAcademicYear->semester)) : '' }}</p>
            </div>
        </div>
        <div class="space-y-4">
            ...
            @endphp
            <div class="bg-primary-50/50 dark:bg-primary-900/20 p-6 rounded-2xl border border-primary-100/50 dark:border-primary-800/50">
                <div class="flex items-start justify-between mb-4">
                    <div class="flex items-start gap-4">
                        <div class="w-12 h-12 bg-primary-100 dark:bg-primary-900/50 rounded-xl flex items-center justify-center">
                            <span class="text-xs font-black text-primary-700 dark:text-primary-300">{{ $class->course->course_code }}</span>
                        </div>
                        <div>
                            <h4 class="font-black text-primary-900 dark:text-white">{{ $class->course->course_name }}</h4>
                            <p class="text-sm text-primary-500 dark:text-slate-400">{{ $class->lecturer->user->name ?? '' }}</p>
                        </div>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <p class="text-xs font-black text-primary-400 uppercase tracking-wider mb-2">{{ __('Attendance') }}</p>
                        <div class="flex items-center gap-3">
                            <p class="text-sm font-black text-primary-900 dark:text-white">{{ $attendanceSummary['present'] }} <span class="text-primary-400 font-medium">/{{ $attendanceSummary['total_meetings'] }}</span></p>
                            <div class="flex-1 bg-primary-100 dark:bg-primary-800 rounded-full h-2 overflow-hidden">
                    ...
                    <div>
                        <p class="text-xs font-black text-primary-400 uppercase tracking-wider mb-2">{{ __('Assignments') }}</p>
                        <div class="flex items-center gap-3">
                            <p class="text-sm font-black text-primary-900 dark:text-white">{{ $assignmentSummary['submitted'] }} <span class="text-primary-400 font-medium">/{{ $assignmentSummary['total'] }}</span></p>
                            <div class="flex-1 bg-primary-100 dark:bg-primary-800 rounded-full h-2 overflow-hidden">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="card-saas p-8">
            <div class="flex items-center gap-3 mb-6">
                ...
                <h3 class="text-base font-black text-primary-900 dark:text-white">{{ __('Passed Subjects') }} ({{ $passedSubjects->count() }})</h3>
            </div>
            @if($passedSubjects->count() > 0)
            <div class="space-y-3 max-h-96 overflow-y-auto pr-2">
                @foreach($passedSubjects as $grade)
                <div class="flex items-center justify-between bg-primary-50/50 dark:bg-primary-900/20 p-4 rounded-xl border border-primary-100/50 dark:border-primary-800/50">
                    <div>
                        <p class="font-black text-primary-900 dark:text-white text-sm">{{ $grade->academicClass->course->course_name }}</p>
                        <p class="text-xs text-primary-500 dark:text-slate-400">{{ $grade->academicClass->academicYear->year ?? '' }} {{ ucfirst(__($grade->academicClass->academicYear->semester ?? '')) }}</p>
                    </div>
            ...
            @else
            <p class="text-primary-400 text-sm">{{ __('No passed subjects yet.') }}</p>
            @endif
        </div>
        ...
        <div class="card-saas p-8">
            <div class="flex items-center gap-3 mb-6">
                ...
                <h3 class="text-base font-black text-primary-900 dark:text-white">{{ __('Failed Subjects') }} ({{ $failedSubjects->count() }})</h3>
            </div>
            ...
            @else
            <p class="text-primary-400 text-sm">{{ __('Great! No failed subjects.') }}</p>
            @endif
        </div>
    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function renderRecommendations(data) {
                ...
                if (data.recommendations && data.recommendations.length > 0) {
                    html += `
                        <h4 class="text-sm font-black text-primary-900 dark:text-white mb-4">{{ __('Recommendations') }}</h4>
                        <div class="space-y-4 mb-6">
                    `;
                    data.recommendations.forEach(rec => {
                        html += `
                            <div class="p-4 bg-primary-50 dark:bg-primary-900/20 rounded-xl border border-primary-100 dark:border-primary-800/50">
                                <h5 class="text-sm font-bold text-primary-900 dark:text-white mb-2">${rec.title}</h5>
                                <p class="text-sm text-primary-600 dark:text-slate-400">${rec.description}</p>
                            </div>
                        `;
                    });
                    html += `</div>`;
                ...
                if (data.study_tips && data.study_tips.length > 0) {
                    html += `
                        <h4 class="text-sm font-black text-primary-900 dark:text-white mb-4">{{ __('Study Tips') }}</h4>
