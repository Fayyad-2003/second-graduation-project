<x-app-layout>
    ...
    <div class="mb-6">
        <a href="{{ route('students.grade-report.index') }}" class="inline-flex items-center gap-2 text-siakad-secondary hover:text-siakad-primary transition">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
                ...
                <div class="mt-4 flex flex-col md:flex-row gap-4 md:items-center md:gap-6">
                    <div>
                        <p class="text-[10px] md:text-xs text-siakad-secondary uppercase tracking-wider mb-0.5 md:mb-1">{{ __('Name') }}</p>
                        <p class="font-semibold text-sm md:text-base">{{ $student->user->name }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] md:text-xs text-siakad-secondary uppercase tracking-wider mb-0.5 md:mb-1">{{ __('Student ID') }}</p>
                        <p class="font-semibold text-sm md:text-base">{{ $student->student_number }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] md:text-xs text-siakad-secondary uppercase tracking-wider mb-0.5 md:mb-1">{{ __('Study Program') }}</p>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mb-8">
        ...
        <div class="card-saas p-5 text-center">
            ...
            <p class="text-2xl font-bold text-siakad-dark">{{ number_format($semesterGpaData['semester_gpa'], 2) }}</p>
            <p class="text-sm text-siakad-secondary">{{ __('GPA') }}</p>
        </div>
        <div class="card-saas p-5 text-center">
            ...
            <p class="text-2xl font-bold text-siakad-dark">{{ number_format($cgpaData['gpa'], 2) }}</p>
            <p class="text-sm text-siakad-secondary">{{ __('Cumulative GPA') }}</p>
        </div>
        <div class="card-saas p-5 text-center">
            ...
            <p class="text-2xl font-bold text-siakad-dark">{{ $gradeList->count() }}</p>
            <p class="text-sm text-siakad-secondary">{{ __('Courses') }}</p>
        </div>
        <div class="card-saas p-5 text-center">
            ...
            <p class="text-2xl font-bold text-siakad-dark">{{ $semesterGpaData['total_credits'] }}</p>
            <p class="text-sm text-siakad-secondary">{{ __('Total Credits') }}</p>
        </div>
    </div>
        ...
        <div class="lg:col-span-2">
            <div class="card-saas overflow-hidden">
                <div class="px-6 py-4 border-b border-siakad-light flex items-center justify-between">
                    <h3 class="font-semibold text-siakad-dark">{{ __('Grade List') }}</h3>
                    <a href="{{ route('students.export.grades', $academicYear) }}" class="inline-flex items-center gap-2 px-3 py-1.5 bg-siakad-primary/10 text-siakad-primary rounded-lg text-sm font-medium hover:bg-siakad-primary/20 transition">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="p-12 text-center">
                    <p class="text-siakad-secondary">{{ __('No grades for this semester yet') }}</p>
                </div>
                @else
                    ...
                    <table class="w-full table-saas">
                        ...
                        <tbody class="divide-y divide-siakad-light">
                            ...
                            <tr class="hover:bg-siakad-light/20 transition">
                                <td class="px-6 py-4">
                                    <span class="font-mono text-sm bg-siakad-light px-2 py-1 rounded text-siakad-dark">{{ $mk->course_code }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <p class="font-medium text-siakad-dark">{{ $mk->course_name }}</p>
                                    <p class="text-xs text-siakad-secondary">{{ $grade->academicClass->lecturer->user->name ?? '-' }}</p>
                                </td>
                                <td class="px-6 py-4 text-center text-siakad-secondary">{{ $mk->credits }}</td>
                                <td class="px-6 py-4 text-center">
                                    <span class="text-siakad-dark font-medium">{{ $grade->numeric_grade ?? '-' }}</span>
                                </td>
                                <td class="px-6 py-4 text-center">
                                ...
                                <td class="px-6 py-4 text-center text-siakad-secondary">{{ number_format($weight * $mk->credits, 1) }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                        <tfoot>
        ...
        <div>
            <div class="card-saas overflow-hidden">
                <div class="px-6 py-4 border-b border-siakad-light">
                    <h3 class="font-semibold text-siakad-dark">{{ __('Grade Distribution') }}</h3>
                </div>
