...
<head>
    ...
    <title>Grade Report {{ $academicYear->year }} Semester {{ __($academicYear->semester) }} - {{ $student->student_number }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="header">
        <h1>{{ __('Kulim University') }}</h1>
        <h2>{{ $student->studyProgram->faculty->name ?? __('FACULTY') }}</h2>
        <p>{{ __('University Address') }}</p>
    </div>

    <div class="title">{{ __('Study Result Card') }}</div>
    <div class="subtitle">{{ __('Academic Year') }} {{ $academicYear->year }} - {{ __('Semester') }} {{ __($academicYear->semester) }}</div>
    ...
    <table class="grade">
        <thead>
            <tr>
                ...
                <th style="width: 60px;">{{ __('Numeric Grade') }}</th>
                <th style="width: 60px;">{{ __('Letter Grade') }}</th>
                <th style="width: 60px;">{{ __('Weight') }}</th>
            </tr>
        </thead>
        <tbody>
            @php $totalCredits = 0; $totalWeight = 0; @endphp
            @forelse($gradeList as $index => $grade)
            @php
        ...
        <tfoot>
            <tr>
                ...
                <td colspan="2" class="center">{{ __('GPA') }}</td>
                <td class="center"><strong>{{ number_format($gpaData['gpa'], 2) }}</strong></td>
            </tr>
        </tfoot>
    </table>
    ...
    <div class="summary">
        <div class="summary-box">
            <div class="value">{{ number_format($gpaData['gpa'], 2) }}</div>
            <div class="label">{{ __('GPA this semester') }}</div>
        </div>
        <div class="summary-box">
        ...
        <div class="summary-box">
            <div class="value">{{ $gpaData['total_credits'] }}</div>
            <div class="label">{{ __('Credits this semester') }}</div>
        </div>
        <div class="summary-box">
            <div class="value">{{ $cgpaData['total_credits'] }}</div>
            <div class="label">{{ __('Cumulative Credits') }}</div>
        </div>
    </div>
    ...
    <div class="footer">
        <div class="signature">
            ...
            Academic Advisor
            <div class="line">
                <strong>{{ $student->academicAdvisor->user->name ?? '_______________________' }}</strong><br>
                {{ __('Lecturer Number') }}. ___________________
            </div>
        </div>
        <div class="signature">
            Academic City, {{ now()->format('d F Y') }}<br>
            {{ __('Head of Study Program') }}
            <div class="line">
