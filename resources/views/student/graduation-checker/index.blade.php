<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto">
            ...
            <div class="flex flex-col md:flex-row items-center gap-6 relative z-10">
                ...
                <div class="text-center md:text-left flex-1">
                    <div class="flex flex-wrap items-center justify-center md:justify-start gap-2 mb-2">
                        ...
                        <span class="text-xs text-siakad-secondary font-mono">{{ $student->student_number }}</span>
                    </div>
                    
                    <h2 class="text-2xl font-bold text-siakad-dark mb-2">
                        @if($eligible)
                    ...
                    <p class="text-sm text-siakad-secondary max-w-2xl">
                        @if($eligible)
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="card-saas p-6">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 bg-siakad-primary/10 rounded-xl flex items-center justify-center flex-shrink-0">
                        <svg class="w-6 h-6 text-siakad-primary" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div>
                        <p class="text-xs text-siakad-secondary uppercase font-semibold tracking-wider">{{ __('Cumulative GPA') }}</p>
                        <p class="text-2xl font-bold text-siakad-dark mt-0.5">{{ number_format($cgpaData['gpa'] ?? 0, 2) }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="card-saas p-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs text-siakad-secondary uppercase font-semibold tracking-wider">{{ __('Total Passed Credits') }}</p>
                        <p class="text-2xl font-bold text-siakad-dark mt-0.5">{{ $totalCreditsPassed }} / {{ $facultyRequiredCredits }} {{ __('Credits') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="card-saas p-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs text-siakad-secondary uppercase font-semibold tracking-wider">{{ __('Remaining Credits') }}</p>
                        <p class="text-2xl font-bold text-siakad-dark mt-0.5">
        ...
        <div class="card-saas p-6 mb-8">
            <h3 class="text-lg font-bold text-siakad-dark mb-4 flex items-center gap-2">
            ...
            <div class="flex justify-between items-end mb-2">
                <span class="text-sm font-medium text-siakad-secondary">{{ __('Progress to Target') }}</span>
                <span class="text-sm font-bold text-siakad-dark">{{ $overallPercentage }}%</span>
            </div>
            
            <div class="w-full bg-siakad-light/50 dark:bg-gray-700 rounded-full h-4 overflow-hidden mb-4">
                <div class="bg-gradient-to-r from-siakad-primary to-siakad-secondary h-full rounded-full transition-all duration-500" style="width: {{ $overallPercentage }}%"></div>
            </div>

            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-siakad-light/50 text-center">
                <div>
                    <span class="text-xs text-siakad-secondary block">{{ __('Faculty Requirements') }}</span>
                    <span class="font-bold text-siakad-dark text-base mt-1 block">{{ $facultyRequiredCredits }} Credits</span>
                </div>
                <div>
                    <span class="text-xs text-siakad-secondary block">{{ __('Credits Passed') }}</span>
                    <span class="font-bold text-emerald-600 dark:text-emerald-400 text-base mt-1 block">{{ $totalCreditsPassed }} Credits</span>
                </div>
                <div>
                    <span class="text-xs text-siakad-secondary block">{{ __('Remaining Target') }}</span>
                    <span class="font-bold text-siakad-dark text-base mt-1 block">{{ $overallRemaining }} Credits</span>
                </div>
                <div>
                    <span class="text-xs text-siakad-secondary block">{{ __('Status') }}</span>
        ...
        <div class="card-saas p-6">
            <h3 class="text-lg font-bold text-siakad-dark mb-6 flex items-center gap-2">
                <svg class="w-5 h-5 text-siakad-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if(count($classificationsData) > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    @foreach($classificationsData as $classification)
                        ...
                        @endphp
                        <div class="card-saas p-5 hover:border-siakad-primary/30 transition-all duration-300 relative overflow-hidden group">
                            ...
                            <div class="flex items-center justify-between mb-3">
                                <h4 class="font-bold text-siakad-dark text-base">{{ $classification['name'] }}</h4>
                            ...
                            <div class="flex justify-between items-end mb-1 text-xs text-siakad-secondary font-medium">
                                <span>{{ __('Completed') }}: <span class="font-semibold text-siakad-dark">{{ $classification['completed'] }}</span> / {{ $classification['required'] }} Credits</span>
                                <span>{{ $classification['percentage'] }}%</span>
                            </div>

                            <!-- Progress Bar -->
                            <div class="w-full bg-siakad-light/50 dark:bg-gray-700 rounded-full h-2 overflow-hidden mb-2">
                                <div class="h-full rounded-full transition-all duration-500 {{ $isMet ? 'bg-emerald-500' : 'bg-siakad-primary' }}" style="width: {{ $classification['percentage'] }}%"></div>
                            </div>

                            <!-- Additional details -->
                            <div class="flex justify-between items-center text-[10px] text-siakad-secondary">
                                <div>
            ...
            @else
                <div class="text-center py-8">
                    <p class="text-sm text-siakad-secondary">{{ __('No classification requirements have been defined for your faculty.') }}</p>
                </div>
            @endif
        </div>
    </div>
