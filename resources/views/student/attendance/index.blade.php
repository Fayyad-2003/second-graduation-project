<x-app-layout>
    ...
    foreach ($summaryBySemester as $semester => $items) {
    $coursesInSemester = $items->map(function($item) {
    $class = $item['class'];
    $summary = $item['summary'];
    $schedule = $class->schedule->first();
    ...
    $today = \Carbon\Carbon::now()->locale('ar')->isoFormat('dddd');
    $isToday = $schedule && $schedule->day == $today;

    return [
    'id' => $class->id,
    'name' => $class->course->course_name,
    'code' => $class->course->course_code,
    'lecturer' => $class->lecturer->user->name ?? __('Lecturer not specified'),
    ...
    'done_meetings' => $summary['present'] + $summary['sick'] + $summary['excused'] + $summary['absent'],
    'time' => $schedule ? substr($schedule->start_time, 0, 5) . ' - ' . substr($schedule->end_time, 0, 5) : __('No schedule yet'),
    'room' => $schedule->room ?? '-',
    'day' => $schedule->day ?? '-',
    'is_today' => $isToday,
    'url' => route('students.attendance.show', $class->id),
    'color' => $color,
    'bgBar' => $bgBar,
    ];
    })->values();

    $attendanceBySemester[$semester] = $coursesInSemester;
    }

    // Also prepare flat list for "today" filter
    $coursesData = collect($attendanceBySemester)->flatten(1)->values();
    @endphp
    ...
    <div x-data="{
        ...
        <div>
            ...
            <div x-show="filteredCourses.length === 0" class="text-center py-20 card-saas" style="display: none;">
                ...
                <h3 class="text-lg font-bold text-slate-700">{{ __('No courses found') }}</h3>
                <p class="text-slate-400 mt-1 max-w-xs mx-auto">{{ __('Try changing your search keywords or filters.') }}</p>
                            ...
                            <div class="bg-siakad-light/30 dark:bg-slate-700/30 rounded-xl p-5 hover:shadow-md transition-all duration-300 group relative">
                                <div class="flex justify-between items-start mb-4">
                                    ...
                                    <div class="text-right">
                                        ...
                                        <p class="text-[10px] font-medium text-slate-400 uppercase tracking-wide">{{ __('Presence') }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center justify-between pt-3 border-t border-white/50 dark:border-slate-600">
                                    <div class="text-xs text-slate-400 font-medium">
                                        {{ $course['done_meetings'] }} {{ __('of') }} {{ $course['total_meetings'] }} {{ __('Meetings') }}
                                    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-[#234C6A] text-white text-xs uppercase tracking-wider">
                                <th class="p-4 font-semibold">{{ __('Course') }}</th>
                                <th class="p-4 font-semibold w-32">{{ __('Lecturer') }}</th>
                                <th class="p-4 font-semibold text-center w-24">{{ __('Credits') }}</th>
                                <th class="p-4 font-semibold text-center w-32">{{ __('Presence') }}</th>
                                <th class="p-4 font-semibold w-40">{{ __('Details') }}</th>
                                <th class="p-4 font-semibold text-right w-24">{{ __('Actions') }}</th>
                            </tr>
                        </thead>
                            ...
                            <template x-for="course in paginatedCourses" :key="'list-'+course.id">
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group">
                                    ...
                                    <td class="p-4 text-center">
                                        <span class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-bold" x-text="course.credits"></span>
                                    </td>
                                    <td class="p-4 text-center">
