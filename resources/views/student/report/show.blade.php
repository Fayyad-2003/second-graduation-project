<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="card-saas dark:bg-gray-800 p-6">
            ...
            <div class="prose dark:prose-invert max-w-none text-siakad-dark dark:text-gray-300 whitespace-pre-wrap">{{ $report->message }}</div>
        </div>

        <!-- Admin Reply -->
        @if($report->admin_reply)
            <div class="card-saas bg-siakad-primary/5 dark:bg-siakad-primary/10 border-siakad-primary/20 p-6">
                <div class="flex items-center gap-3 mb-4">
                    <div class="w-10 h-10 rounded-full bg-siakad-primary flex items-center justify-center text-white">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                        </svg>
                    </div>
                    <div>
                        <h3 class="font-bold text-siakad-dark dark:text-white">{{ __('Admin Reply') }}</h3>
        ...
        @elseif($report->status === 'pending')
            <div class="text-center p-8 bg-gray-50 dark:bg-gray-900/30 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700">
                <p class="text-siakad-secondary dark:text-gray-400">{{ __('Your report is waiting for review from admin.') }}</p>
            </div>
        @endif

        <div class="flex justify-start">
            <a href="{{ route('students.report.index') }}" class="flex items-center gap-2 text-sm text-siakad-secondary dark:text-gray-400 hover:text-siakad-primary transition">
