<x-app-layout>
    ...
    <div class="mb-6">
        <p class="text-sm text-primary-secondary">{{ __('Manage your practical work and logbook') }}</p>
    </div>
    ...
    <div class="card-saas p-12 text-center">
        ...
        <h3 class="text-xl font-bold text-primary-dark mb-2">{{ __('No KP application submitted yet') }}</h3>
        <p class="text-primary-secondary mb-6">{{ __('Apply for practical work to start') }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div class="lg:col-span-2 card-saas p-6">
            <div class="flex items-start justify-between mb-4">
                <div>
                    ...
                    <h2 class="text-lg font-bold text-primary-dark mt-3">{{ $internship->company_name }}</h2>
                    <p class="text-sm text-primary-secondary">{{ $internship->business_field }}</p>
                </div>
            </div>
            ...
            <div class="mt-4">
                <div class="flex items-center justify-between text-sm mb-2">
                    ...
                    <span class="font-semibold text-primary-primary">{{ $internship->progress_percent }}%</span>
                </div>
                <div class="h-3 bg-primary-light rounded-full overflow-hidden">
                    <div class="h-full bg-primary-primary rounded-full transition-all" style="width: {{ $internship->progress_percent }}%"></div>
                </div>
            </div>

            <div class="grid grid-cols-2 gap-4 mt-6 text-sm">
                <div class="p-4 rounded-xl bg-primary-light/30">
                    <p class="text-xs text-primary-secondary mb-1">{{ __('Period') }}</p>
                    <p class="font-medium text-primary-dark">{{ $internship->start_date->format('d M') }} - {{ $internship->completion_date->format('d M Y') }}</p>
                    <p class="text-xs text-primary-secondary">({{ $internship->duration }})</p>
                </div>
                <div class="p-4 rounded-xl bg-primary-light/30">
                    <p class="text-xs text-primary-secondary mb-1">{{ __('Campus Advisor') }}</p>
                    <p class="font-medium text-primary-dark">{{ $internship->supervisor?->user->name ?? __('Not yet specified') }}</p>
                </div>
                <div class="p-4 rounded-xl bg-primary-light/30">
                    <p class="text-xs text-primary-secondary mb-1">{{ __('Field Supervisor') }}</p>
                    <p class="font-medium text-primary-dark">{{ $internship->field_supervisor_name ?? '-' }}</p>
                </div>
                <div class="p-4 rounded-xl bg-primary-light/30">
                    <p class="text-xs text-primary-secondary mb-1">{{ __('Address') }}</p>
                    <p class="font-medium text-primary-dark text-xs">{{ $internship->company_address ?? '-' }}</p>
                </div>
            </div>
        </div>
            ...
            <div class="space-y-0">
                ...
                <div class="flex-1 pb-4">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-xs {{ $isCompleted ? 'text-primary-secondary' : 'text-slate-400' }}">
        ...
        <div class="mt-4 pt-4 border-t border-primary-light">
            <div class="text-center">
                <p class="text-3xl font-bold text-primary-primary">{{ $logbookList->count() }}</p>
                <p class="text-sm text-primary-secondary">{{ __('Logbook Entries') }}</p>
            </div>
        </div>
    </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="card-saas p-5 flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-bold text-primary-dark">{{ $logbookList->count() }}</p>
                <p class="text-sm text-primary-secondary">{{ __('Total Logbook') }}</p>
            </div>
        </div>
        <div class="card-saas p-5 flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-bold text-primary-dark">{{ $internship->duration }}</p>
                <p class="text-sm text-primary-secondary">{{ __('KP Duration') }}</p>
            </div>
        </div>
        <div class="card-saas p-5 flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-bold text-primary-dark">{{ $internship->progress_percent }}%</p>
                <p class="text-sm text-primary-secondary">{{ __('Progress') }}</p>
            </div>
        </div>
        <div class="card-saas p-5 flex items-center gap-4">
            ...
            <div>
                ...
                @endphp
                <p class="text-2xl font-bold text-primary-dark">{{ $daysLeft > 0 ? $daysLeft : 0 }}</p>
                <p class="text-sm text-primary-secondary">{{ __('Days Left') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 card-saas overflow-hidden">
            <div class="px-6 py-4 border-b border-primary-light flex items-center justify-between">
                <h3 class="font-semibold text-primary-dark">{{ __('Activity Log') }}</h3>
                <span class="text-sm text-primary-secondary">{{ $logbookList->count() }} {{ __('entries') }}</span>
            </div>
            @forelse($logbookList as $log)
            <div class="p-5 border-b border-primary-light/50">
                <div class="flex items-start gap-4">
                    <div class="w-12 h-12 rounded-lg bg-primary-primary/10 flex flex-col items-center justify-center flex-shrink-0">
                        <p class="text-lg font-bold text-primary-primary leading-none">{{ $log->date->format('d') }}</p>
                        <p class="text-[10px] text-primary-secondary">{{ $log->date->format('M') }}</p>
                    </div>
                    <div class="flex-1">
                        <div class="flex items-center gap-2 mb-2">
                            <span class="text-xs text-primary-secondary">{{ $log->entry_time ?? '-' }} - {{ $log->exit_time ?? '-' }}</span>
                            <span class="px-2 py-0.5 text-xs rounded-full bg-{{ $log->status_color }}-100 text-{{ $log->status_color }}-700">{{ $log->status_label }}</span>
                        </div>
                        <p class="text-sm text-primary-dark">{{ $log->activity }}</p>
        ...
        <!-- Add Logbook Form -->
        <div class="card-saas p-6">
            <h3 class="font-semibold text-primary-dark mb-4">{{ __('Add Logbook') }}</h3>
            <form action="{{ route('students.internship.logbook.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <div>
                    <label class="block text-xs font-medium text-primary-dark mb-1">{{ __('Date') }}</label>
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div>
                        <label class="block text-xs font-medium text-primary-dark mb-1">{{ __('Entry Time') }}</label>
                        <input type="time" name="entry_time" class="input-saas w-full text-sm" style="background-color: var(--bg-card);">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-primary-dark mb-1">{{ __('Exit Time') }}</label>
                        <input type="time" name="exit_time" class="input-saas w-full text-sm" style="background-color: var(--bg-card);">
                    </div>
                </div>
                <div>
                    <label class="block text-xs font-medium text-primary-dark mb-1">{{ __('Activity') }}</label>
                ...
                <!-- Modern File Upload -->
                <div x-data="{ fileName: '' }">
                    <label class="block text-xs font-medium text-primary-dark mb-1">{{ __('Documentation (Optional)') }}</label>
                    <label class="relative flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-primary-light dark:border-slate-600 rounded-xl cursor-pointer hover:border-[#234C6A] hover:bg-[#234C6A]/5 transition-all">
                        <div class="flex flex-col items-center justify-center text-center px-4">
                            <svg class="w-6 h-6 text-primary-secondary mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                            </svg>
                            <p class="text-xs text-primary-secondary" x-show="!fileName">{{ __('Upload Images') }}</p>
                            <p class="text-xs font-medium text-[#234C6A]" x-show="fileName" x-text="fileName"></p>
                            <p class="text-[10px] text-primary-secondary mt-0.5">JPG, PNG (max 5MB)</p>
                        </div>
