<x-app-layout>
    ...
    <div class="mb-6">
        <h2 class="text-xl font-bold text-siakad-dark dark:text-white">{{ __('My Classes') }}</h2>
        <p class="text-sm text-siakad-secondary dark:text-gray-400">{{ __('Access materials and assignments from the classes you take') }}</p>
    </div>
    ...
    <div class="mb-6 flex flex-wrap items-center gap-2">
        <a href="{{ route('students.lms.index', ['semester' => 'active']) }}" 
           class="px-4 py-2 rounded-lg text-sm font-medium transition {{ $semesterFilter === 'active' ? 'bg-siakad-primary text-white' : 'bg-gray-100 dark:bg-gray-700 text-siakad-dark dark:text-white hover:bg-gray-200 dark:hover:bg-gray-600' }}">
        ...
        <div class="relative">
            <select id="semesterDropdown" onchange="if(this.value) window.location.href=this.value" 
                    class="appearance-none pl-4 pr-10 py-2 rounded-lg text-sm font-medium transition cursor-pointer {{ $semesterFilter !== 'active' ? 'bg-siakad-primary text-white' : 'bg-gray-100 dark:bg-gray-700 text-siakad-dark dark:text-white hover:bg-gray-200 dark:hover:bg-gray-600' }}">
            ...
            <svg class="w-4 h-4 absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none {{ $semesterFilter !== 'active' ? 'text-white' : 'text-siakad-dark dark:text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
        </div>
        @endif
    </div>
    @else
    ...
    <div class="mb-6">
        ...
        <div class="flex items-center gap-3">
            <label class="text-sm text-siakad-secondary dark:text-gray-400">{{ __('Select Semester') }}:</label>
            <div class="relative">
                <select id="semesterDropdown" onchange="if(this.value) window.location.href=this.value" 
    ...
    <div class="card-saas p-8 text-center dark:bg-gray-800">
        ...
        <p class="text-siakad-secondary dark:text-gray-400">{{ __('No classes this semester.') }}</p>
        <a href="{{ route('students.lms.index', ['semester' => 'active']) }}" class="inline-block mt-3 text-siakad-primary hover:underline text-sm">&larr; {{ __('Back to Active Semester') }}</a>
        @else
        <p class="text-siakad-secondary dark:text-gray-400">{{ __("You haven't registered for any classes yet.") }}</p>
        <a href="{{ route('students.study-plan.index') }}" class="inline-block mt-3 text-siakad-primary hover:underline text-sm">{{ __('Fill Study Plan') }} &rarr;</a>
        @endif
    </div>
    @else
    ...
    <div id="noResults" class="hidden card-saas p-8 text-center dark:bg-gray-800">
        ...
        <p class="text-siakad-secondary dark:text-gray-400">{{ __('No classes found.') }}</p>
    </div>
    @endif
