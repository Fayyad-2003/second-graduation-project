<x-app-layout>
    <div class="mb-10">
        <h1 class="text-3xl font-black tracking-tight text-siakad-900 dark:text-white">{{ __('Document Applications') }}</h1>
        ...
        <div class="card-saas p-5 relative overflow-hidden group">
            <div class="flex items-center gap-4 relative z-10">
                <div class="w-10 h-10 rounded-xl bg-{{ $stat['color'] }}-50 dark:bg-{{ $stat['color'] }}-950/30 flex items-center justify-center text-{{ $stat['color'] }}-600 dark:text-{{ $stat['color'] }}-400 shadow-sm border border-{{ $stat['color'] }}-100/50 dark:border-{{ $stat['color'] }}-900/50">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="{{ $stat['icon'] }}"></path></svg>
                </div>
                <div>
                    <p class="text-xl font-black text-siakad-900 dark:text-white leading-tight">{{ $stat['count'] }}</p>
                    <p class="text-[9px] text-siakad-400 font-black uppercase tracking-widest mt-0.5">{{ $stat['label'] }}</p>
                </div>
            </div>
        </div>
        @endforeach
            ...
            <table class="w-full text-start border-collapse">
                <thead>
                    <tr class="bg-siakad-50/50 dark:bg-siakad-900/30 border-b border-siakad-100/50 dark:border-siakad-800/50">
                        <th class="py-5 px-8 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start">{{ __('Student') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start">{{ __('Document Type') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center">{{ __('Status') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center">{{ __('Applied Date') }}</th>
                    ...
                    <tr class="hover:bg-siakad-50/30 dark:hover:bg-siakad-900/20 transition-colors group">
                        <td class="py-5 px-8 text-start">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="min-w-0">
                                    <p class="text-sm font-black text-siakad-900 dark:text-white truncate leading-tight">{{ $app->student->user->name ?? '-' }}</p>
                                    <p class="text-[10px] text-siakad-400 font-mono font-bold uppercase tracking-wider mt-0.5">{{ $app->student->student_number }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="py-5 px-6 text-start">
                            <p class="text-xs font-bold text-siakad-600 dark:text-siakad-400 leading-tight">{{ $app->documentType->name }}</p>
                        </td>
                        <td class="py-5 px-6 text-center">
                            @php
                                ...
                                $statusClass = $statusClasses[$app->status] ?? 'bg-siakad-50 text-siakad-600 dark:bg-siakad-950/30 dark:text-siakad-400 border-siakad-100 dark:border-siakad-900/50';
                            @endphp
                        ...
                        <td class="py-5 px-6 text-center">
                            <p class="text-xs font-bold text-siakad-400 dark:text-siakad-500 font-mono">{{ $app->created_at->format('d M Y') }}</p>
                            <p class="text-[9px] font-black text-siakad-300 dark:text-siakad-600 uppercase tracking-tighter">{{ $app->created_at->format('H:i') }}</p>
                        </td>
                        <td class="py-5 px-8 text-end">
                            <a href="{{ route('admin.document-application.show', $app) }}" 
                                class="btn-ghost-saas px-4 py-2 rounded-xl text-[10px] font-black flex items-center gap-2 border border-siakad-100 dark:border-siakad-800 text-siakad-primary hover:bg-siakad-primary/10 transition-all inline-flex uppercase tracking-widest">
                    ...
                    <tr>
                        <td colspan="5" class="py-16 text-center">
                            <div class="flex flex-col items-center">
                                <div class="w-16 h-16 bg-siakad-50 dark:bg-siakad-900/50 rounded-2xl flex items-center justify-center mb-4 text-siakad-200">
                                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                </div>
                                <p class="text-siakad-400 font-bold uppercase tracking-widest text-[10px]">{{ __('No applications found') }}</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
            ...
            <div class="p-5 hover:bg-siakad-50/30 dark:hover:bg-siakad-900/20 transition-colors">
                <div class="flex items-start justify-between gap-4 mb-4">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="min-w-0">
                            <h4 class="text-sm font-black text-siakad-900 dark:text-white truncate leading-tight">{{ $app->student->user->name ?? '-' }}</h4>
                            <p class="text-[10px] text-siakad-400 font-mono font-bold mt-0.5 tracking-wider">{{ $app->student->student_number }}</p>
                        </div>
                    </div>
                    @php
                ...
                <div class="bg-siakad-50/50 dark:bg-siakad-900/30 p-3 rounded-2xl border border-siakad-100/50 dark:border-siakad-800/50 mb-4">
                    <div class="flex justify-between items-center mb-2">
                        <span class="text-[8px] font-black text-siakad-400 uppercase tracking-widest">{{ __('Document') }}</span>
                        <span class="text-xs font-bold text-siakad-700 dark:text-siakad-300">{{ $app->documentType->name }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-[8px] font-black text-siakad-400 uppercase tracking-widest">{{ __('Applied') }}</span>
                        <span class="text-[10px] font-bold text-siakad-500 dark:text-siakad-400 font-mono">{{ $app->created_at->format('d M Y, H:i') }}</span>
                    </div>
                </div>

                <a href="{{ route('admin.document-application.show', $app) }}" 
                    class="btn-ghost-saas w-full py-3 rounded-xl text-[10px] font-black flex items-center justify-center gap-2 border border-siakad-100 dark:border-siakad-800 text-siakad-primary uppercase tracking-widest">
