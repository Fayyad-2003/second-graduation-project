<x-app-layout>
    <div class="mb-10">
        <h1 class="text-3xl font-black tracking-tight text-siakad-900 dark:text-white">{{ __('Lecturer Attendance Monitoring') }}</h1>
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-6 mb-10">
        ...
        <div class="card-saas p-5 relative overflow-hidden group">
            <div class="flex flex-col items-center text-center relative z-10">
                ...
                <p class="text-2xl font-black text-siakad-900 dark:text-white leading-tight">{{ $s['1'] }}</p>
                <p class="text-[9px] text-siakad-400 font-black uppercase tracking-widest mt-1">{{ $s['0'] }}</p>
            </div>
        </div>
        @endforeach
    </div>
    ...
    <div class="card-saas p-6 mb-10">
        <form method="GET" class="flex flex-wrap items-end gap-4">
            <div class="flex-1 min-w-[200px]">
                <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1 mb-1.5 block">{{ __('Lecturer') }}</label>
            ...
            <div class="w-full sm:w-48">
                <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1 mb-1.5 block">{{ __('Month') }}</label>
            ...
            <div class="w-full sm:w-32">
                <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1 mb-1.5 block">{{ __('Year') }}</label>
    ...
    <div class="card-saas mb-10 overflow-hidden">
        <div class="px-8 py-6 border-b border-siakad-100/50 dark:border-siakad-800/50 bg-siakad-50/30 dark:bg-siakad-900/20 flex items-center justify-between">
            <h3 class="text-lg font-black text-siakad-900 dark:text-white flex items-center gap-3">
                <div class="w-1.5 h-6 bg-siakad-primary rounded-full"></div>
        ...
        <div class="overflow-x-auto">
            ...
            <table class="hidden md:table w-full text-start border-collapse">
                <thead>
                    <tr class="bg-siakad-50/50 dark:bg-siakad-900/30 border-b border-siakad-100/50 dark:border-siakad-800/50">
                        <th class="py-5 px-8 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start">{{ __('Lecturer') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center">{{ __('Present') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center">{{ __('Permission') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center">{{ __('Sick') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center">{{ __('On Duty') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center">{{ __('Absent') }}</th>
                        <th class="py-5 px-8 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center">{{ __('Attendance Rate') }}</th>
                    </tr>
                </thead>
                ...
                <tr class="hover:bg-siakad-50/30 dark:hover:bg-siakad-900/20 transition-colors group">
                    <td class="py-5 px-8 text-start">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="min-w-0 text-start">
                                <p class="text-sm font-black text-siakad-900 dark:text-white truncate leading-tight">{{ $d->user->name }}</p>
                                <p class="text-[10px] text-siakad-400 font-mono font-bold uppercase tracking-wider mt-0.5">{{ $d->lecturer_number }}</p>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="py-5 px-8 text-center">
                        ...
                        <div class="flex flex-col items-center gap-1.5">
                            ...
                            <div class="w-20 h-1 bg-siakad-50 dark:bg-siakad-900 rounded-full overflow-hidden">
            ...
            <div class="md:hidden divide-y divide-siakad-50 dark:divide-siakad-800/50">
                @foreach($lecturerList as $d)
                @php 
                    $summary = $lecturerSummary[$d->id] ?? collect();
                    $presentCount = $summary->where('status', 'present')->first()?->count ?? 0;
                    ...
                    $rateColor = $percentage >= 80 ? 'emerald' : ($percentage >= 60 ? 'amber' : 'red');
                @endphp
                <div class="p-5 hover:bg-siakad-50/30 dark:hover:bg-siakad-900/20 transition-colors">
                    <div class="flex items-start justify-between mb-4">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="min-w-0">
                                <h4 class="text-sm font-black text-siakad-900 dark:text-white truncate leading-tight">{{ $d->user->name }}</h4>
                                <p class="text-[10px] text-siakad-400 font-mono font-bold mt-0.5 tracking-wider">{{ $d->lecturer_number }}</p>
                            </div>
                        </div>
    ...
    <div class="card-saas overflow-hidden">
            ...
            <table class="hidden md:table w-full text-start border-collapse">
                <thead>
                    <tr class="bg-siakad-50/50 dark:bg-siakad-900/30 border-b border-siakad-100/50 dark:border-siakad-800/50">
                        ...
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start">{{ __('Lecturer') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start">{{ __('Course') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center">{{ __('Time') }}</th>
                        <th class="py-5 px-8 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center">{{ __('Status') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-siakad-50 dark:divide-siakad-800/50">
                ...
                <tr class="hover:bg-siakad-50/30 dark:hover:bg-siakad-900/20 transition-colors group">
                    <td class="py-5 px-8 text-start">
                        <span class="text-xs font-black text-siakad-900 dark:text-white font-mono uppercase tracking-widest">{{ $k->date->format('d M Y') }}</span>
                    </td>
                    <td class="py-5 px-6 text-start">
                        <span class="text-sm font-black text-siakad-900 dark:text-white">{{ $k->lecturer->user->name }}</span>
                    </td>
                    <td class="py-5 px-6 text-start">
                        <p class="text-xs font-bold text-siakad-600 dark:text-siakad-400 leading-tight">{{ $k->courseSchedule?->class?->course?->course_name ?? '-' }}</p>
                    </td>
                    <td class="py-5 px-6 text-center">
                        <span class="text-[10px] font-black text-siakad-400 dark:text-siakad-500 font-mono uppercase tracking-widest">{{ $k->check_in_time ? substr($k->check_in_time, 0, 5) : '--:--' }} — {{ $k->check_out_time ? substr($k->check_out_time, 0, 5) : '--:--' }}</span>
                    </td>
                    <td class="py-5 px-8 text-center">
                ...
                <tr>
                    <td colspan="5" class="py-16 text-center">
                        <p class="text-siakad-400 font-bold uppercase tracking-widest text-[10px]">{{ __('No detailed data available yet') }}</p>
                    </td>
                </tr>
                @endforelse
                </tbody>
            ...
            <div class="md:hidden divide-y divide-siakad-50 dark:divide-siakad-800/50">
                ...
                <div class="p-5 hover:bg-siakad-50/30 dark:hover:bg-siakad-900/20 transition-colors">
                    <div class="flex items-start justify-between mb-3">
                        <div>
                            <span class="text-[10px] font-black text-siakad-400 uppercase tracking-widest">{{ $k->date->format('d M Y') }}</span>
                            <h4 class="font-black text-siakad-900 dark:text-white text-sm mt-1 leading-tight">{{ $k->lecturer->user->name }}</h4>
                        </div>
                    ...
                    @if($k->courseSchedule)
                    <div class="bg-siakad-50/50 dark:bg-siakad-900/30 p-3 rounded-2xl border border-siakad-100/50 dark:border-siakad-800/50">
                        <p class="text-xs font-bold text-siakad-700 dark:text-siakad-300 mb-2 leading-tight">{{ $k->courseSchedule->class->course->course_name ?? '-' }}</p>
                        <div class="flex items-center gap-2 text-[10px] font-black text-siakad-400 uppercase tracking-widest">
        ...
        <div class="px-8 py-6 border-t border-siakad-50/50 dark:border-siakad-800/50 bg-siakad-50/20 dark:bg-siakad-900/10">
