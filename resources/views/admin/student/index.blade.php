<x-app-layout>
    <div class="mb-10">
        <h1 class="text-3xl font-black tracking-tight text-siakad-900 dark:text-white">{{ __('Student Management') }}</h1>
    ...
    <div class="mb-8 card-saas p-6">
        <form method="GET" class="flex flex-wrap items-center gap-4">
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto">
                ...
                <a href="{{ route('admin.student.index') }}" class="btn-ghost-saas px-4 py-2.5 rounded-xl text-sm font-bold text-siakad-400 hover:text-siakad-600">
                    {{ __('Reset') }}
                </a>
                @endif
            </div>
            ...
            <table class="w-full text-start border-collapse">
                ...
                <tbody class="divide-y divide-siakad-50 dark:divide-siakad-800/50">
                    ...
                    <tr class="hover:bg-siakad-50/30 dark:hover:bg-siakad-900/20 transition-colors group">
                        <td class="py-5 px-8 text-xs font-bold text-siakad-400 text-start">{{ $students->firstItem() + $index }}</td>
                        <td class="py-5 px-6 text-start">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="min-w-0">
                                    <p class="text-sm font-black text-siakad-900 dark:text-white truncate">{{ $m->user->name ?? '-' }}</p>
                                    <p class="text-[10px] text-siakad-400 font-bold uppercase tracking-wider text-start">{{ $m->user->email ?? '-' }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="py-5 px-6 text-start">
                            <span class="text-xs font-black text-siakad-700 dark:text-siakad-300 font-mono tracking-wider">{{ $m->student_number }}</span>
                        </td>
                        <td class="py-5 px-6 text-start">
                            <p class="text-xs font-bold text-siakad-600 dark:text-siakad-400">{{ $m->studyProgram->name ?? '-' }}</p>
                        </td>
                        <td class="py-5 px-6 text-center">
                            <span class="inline-flex px-2.5 py-1 text-[10px] font-black bg-siakad-50 dark:bg-siakad-900 text-siakad-500 dark:text-siakad-400 rounded-lg border border-siakad-100 dark:border-siakad-800 uppercase">{{ $m->batch }}</span>
                        </td>
                        <td class="py-5 px-6 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <div class="w-1.5 h-1.5 rounded-full {{ $m->ipk >= 3.0 ? 'bg-emerald-500' : ($m->ipk >= 2.0 ? 'bg-amber-500' : 'bg-red-500') }}"></div>
                                <span class="text-sm font-black text-siakad-900 dark:text-white">{{ number_format($m->ipk ?? 0, 2) }}</span>
                            </div>
                        </td>
                        <td class="py-5 px-6 text-center">
                            @php
                            ...
                            'default' => 'bg-siakad-50 text-siakad-600 dark:bg-siakad-950/30 dark:text-siakad-400 border-siakad-100 dark:border-siakad-900/50'
                            ];
                        ...
                        <td class="py-5 px-8 text-end">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <button
                                    ...
                                    class="p-2 text-siakad-secondary hover:text-siakad-primary hover:bg-siakad-primary/10 rounded-lg transition" title="{{ __('Edit') }}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <tr>
                        <td colspan="8" class="py-12 text-center">
                            <div class="flex flex-col items-center">
                                <div
                                    class="w-12 h-12 bg-siakad-light/50 dark:bg-gray-700/50 rounded-xl flex items-center justify-center mb-3">
                                    <svg class="w-6 h-6 text-siakad-secondary dark:text-gray-400" fill="none"
                                ...
                                <p class="text-siakad-secondary dark:text-gray-400 text-sm">{{ __('No student data available') }}</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
    ...
    <div class="md:hidden space-y-4 mb-6">
        ...
        <div class="card-saas p-6 group">
            <div class="flex items-start justify-between mb-4">
                <div class="flex items-center gap-4">
                    ...
                    <div class="min-w-0">
                        <h4 class="font-black text-siakad-900 dark:text-white truncate group-hover:text-siakad-primary transition-colors">{{ $m->user->name ?? '-' }}</h4>
                        <p class="text-[10px] text-siakad-400 font-black uppercase tracking-widest mt-1">{{ $m->student_number }}</p>
                    </div>
                </div>
                @php
                $statusClasses = [
            ...
            <div class="grid grid-cols-2 gap-3 mb-6">
                <div class="bg-siakad-50/50 dark:bg-siakad-900/50 p-3 rounded-2xl border border-siakad-100/50 dark:border-siakad-800/50">
                    <span class="block text-[10px] text-siakad-400 font-black uppercase tracking-widest mb-1">{{ __('Program') }}</span>
                    <span class="text-xs font-black text-siakad-900 dark:text-white line-clamp-1">{{ $m->studyProgram->name ?? '-' }}</span>
                </div>
                <div class="bg-siakad-50/50 dark:bg-siakad-900/50 p-3 rounded-2xl border border-siakad-100/50 dark:border-siakad-800/50">
                    <span class="block text-[10px] text-siakad-400 font-black uppercase tracking-widest mb-1">{{ __('Batch') }}</span>
                    <span class="text-xs font-black text-siakad-900 dark:text-white">{{ $m->batch }}</span>
                </div>
                <div class="bg-siakad-50/50 dark:bg-siakad-900/50 p-3 rounded-2xl border border-siakad-100/50 dark:border-siakad-800/50 col-span-2">
                    <div class="flex items-center justify-between">
                        <span class="text-[10px] text-siakad-400 font-black uppercase tracking-widest">{{ __('GPA Status') }}</span>
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-siakad-50 dark:border-siakad-800">
                <a href="{{ route('admin.student.show', $m) }}" class="flex-1 py-3 text-xs font-black text-siakad-700 dark:text-siakad-300 bg-siakad-50 dark:bg-siakad-800 rounded-xl hover:bg-siakad-primary hover:text-white transition-all text-center">
                    {{ __('View Details') }}
                </a>
                <button onclick="openEditModal({{ json_encode(['id' => $m->id, 'name' => $m->user->name, 'email' => $m->user->email, 'student_number' => $m->student_number, 'study_program_id' => $m->study_program_id, 'batch' => $m->batch, 'academic_advisor_id' => $m->academic_advisor_id, 'status' => $m->status]) }})"
                    class="p-3 text-siakad-secondary hover:text-siakad-primary hover:bg-siakad-primary/10 rounded-xl transition-all border border-siakad-100 dark:border-siakad-800">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="card-saas p-10 text-center">
            <p class="text-siakad-400 font-bold">{{ __('No student data available') }}</p>
        </div>
        @endforelse
    </div>
                    ...
                    <form action="{{ route('admin.student.store') }}" method="POST">
                        ...
                        <div class="space-y-5 max-h-[60vh] overflow-y-auto pr-2 scrollbar-hide">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-[10px] font-black text-siakad-400 uppercase tracking-widest mb-2 ml-1">{{ __('Email') }}</label>
                                    <input type="email" name="email" class="input-saas w-full px-4 py-3 text-sm rounded-xl" placeholder="student@siakad.com" required>
                                </div>
                                <div>
                            ...
                            <div>
                                <label class="block text-[10px] font-black text-siakad-400 uppercase tracking-widest mb-2 ml-1">{{ __('Student ID Number') }}</label>
                            ...
                            <div>
                                <label class="block text-[10px] font-black text-siakad-400 uppercase tracking-widest mb-2 ml-1">{{ __('Study Program') }}</label>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-[10px] font-black text-siakad-400 uppercase tracking-widest mb-2 ml-1">{{ __('Batch Year') }}</label>
                                    <input type="number" name="batch" value="{{ date('Y') }}" class="input-saas w-full px-4 py-3 text-sm rounded-xl" required>
                                </div>
                                <div>
                                    <label class="block text-[10px] font-black text-siakad-400 uppercase tracking-widest mb-2 ml-1">{{ __('Academic Advisor') }}</label>
                        ...
                        <div class="mt-8 flex items-center justify-end gap-3">
                            ...
                            <button type="submit" class="btn-primary-saas px-8 py-2.5 rounded-xl text-sm font-black shadow-lg shadow-siakad-600/20">{{ __('Save Student') }}</button>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="relative">
                    ...
                    <form id="editForm" method="POST">
                        ...
                        <div class="space-y-5 max-h-[60vh] overflow-y-auto pr-2 scrollbar-hide">
                            ...
                            <div>
                                <label class="block text-[10px] font-black text-siakad-400 uppercase tracking-widest mb-2 ml-1">{{ __('Academic Status') }}</label>
                        ...
                        <div class="mt-8 flex items-center justify-end gap-3">
                            ...
                            <button type="submit" class="btn-primary-saas px-8 py-2.5 rounded-xl text-sm font-black shadow-lg shadow-siakad-600/20">{{ __('Update Student') }}</button>
                        </div>
                    </form>
                </div>
            </div>
