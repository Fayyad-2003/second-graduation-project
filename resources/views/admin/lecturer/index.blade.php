<x-app-layout>
    <div class="mb-10">
        <h1 class="text-3xl font-black tracking-tight text-siakad-900 dark:text-white">{{ __('Lecturer Management') }}</h1>
            ...
            <table class="w-full text-start border-collapse">
                <thead>
                    <tr class="bg-siakad-50/50 dark:bg-siakad-900/30 border-b border-siakad-100/50 dark:border-siakad-800/50">
                        ...
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center">{{ __('Classes') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center">{{ __('Mentoring') }}</th>
                ...
                <tbody class="divide-y divide-siakad-50 dark:divide-siakad-800/50">
                    ...
                    <tr class="hover:bg-siakad-50/30 dark:hover:bg-siakad-900/20 transition-colors group">
                        <td class="py-5 px-8 text-xs font-bold text-siakad-400 text-start">{{ $lecturers->firstItem() + $index }}</td>
                        <td class="py-5 px-6 text-start">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="min-w-0">
                                    <p class="text-sm font-black text-siakad-900 dark:text-white truncate">{{ $d->user->name ?? '-' }}</p>
                                    <p class="text-[10px] text-siakad-400 font-bold uppercase tracking-wider text-start">{{ $d->user->email ?? '-' }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="py-5 px-6 text-start">
                            <span class="text-xs font-black text-siakad-700 dark:text-siakad-300 font-mono tracking-wider bg-siakad-50 dark:bg-siakad-900 px-2 py-1 rounded-lg border border-siakad-100 dark:border-siakad-800">{{ $d->lecturer_number }}</span>
                        </td>
                        <td class="py-5 px-6 text-start">
                            <p class="text-xs font-bold text-siakad-600 dark:text-siakad-400">{{ $d->studyProgram->name ?? '-' }}</p>
                        </td>
                        <td class="py-5 px-6 text-center">
                            <span class="inline-flex px-2.5 py-1 text-[10px] font-black bg-siakad-50 dark:bg-siakad-900 text-siakad-600 dark:text-siakad-400 rounded-lg border border-siakad-100 dark:border-siakad-800 uppercase tracking-widest">{{ $d->classes_count ?? $d->classes->count() }}</span>
                        </td>
                        <td class="py-5 px-6 text-center">
                        ...
                        <td class="py-5 px-8 text-end">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.lecturer.show', $d) }}" class="p-2 text-siakad-secondary hover:text-siakad-primary hover:bg-siakad-primary/10 rounded-lg transition" title="{{ __('Details') }}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                    </svg>
                                </a>
                                <button onclick="openEditModal({{ json_encode(['id'=>$d->id,'name'=>$d->user->name,'email'=>$d->user->email,'lecturer_number'=>$d->lecturer_number,'study_program_id'=>$d->study_program_id]) }})" class="p-2 text-siakad-secondary hover:text-siakad-primary hover:bg-siakad-primary/10 rounded-lg transition" title="{{ __('Edit') }}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <tr>
                        <td colspan="7" class="py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-siakad-secondary dark:text-gray-400 text-sm">{{ __('No lecturer data available') }}</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
    ...
    <div class="md:hidden space-y-4 mb-6">
        ...
        <div class="card-saas p-6 group">
            <div class="flex items-start justify-between mb-4">
                <div class="flex items-center gap-4">
                    ...
                    <div class="min-w-0">
                        <h4 class="font-black text-siakad-900 dark:text-white truncate group-hover:text-siakad-primary transition-colors">{{ $d->user->name ?? '-' }}</h4>
                        <p class="text-[10px] text-siakad-400 font-black uppercase tracking-widest mt-1">{{ $d->lecturer_number }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3 mb-6">
                <div class="bg-siakad-50/50 dark:bg-siakad-900/50 p-3 rounded-2xl border border-siakad-100/50 dark:border-siakad-800/50 col-span-2">
                    ...
                    <span class="text-xs font-black text-siakad-900 dark:text-white line-clamp-1">{{ $d->studyProgram->name ?? '-' }}</span>
                </div>
                <div class="bg-siakad-50/50 dark:bg-siakad-900/50 p-3 rounded-2xl border border-siakad-100/50 dark:border-siakad-800/50">
                    <span class="block text-[10px] text-siakad-400 font-black uppercase tracking-widest mb-1">{{ __('Teaching Classes') }}</span>
                    <span class="text-xs font-black text-siakad-900 dark:text-white">{{ $d->classes_count ?? $d->classes->count() }}</span>
                </div>
                <div class="bg-siakad-50/50 dark:bg-siakad-900/50 p-3 rounded-2xl border border-siakad-100/50 dark:border-siakad-800/50">
                    <span class="block text-[10px] text-siakad-400 font-black uppercase tracking-widest mb-1">{{ __('Mentoring') }}</span>
                    <span class="text-xs font-black text-siakad-900 dark:text-white">{{ $d->advised_students_count ?? $d->advisedStudents->count() }}</span>
                </div>
            </div>

            <div class="flex items-center gap-3 pt-4 border-t border-siakad-50 dark:border-siakad-800">
                <a href="{{ route('admin.lecturer.show', $d) }}" class="flex-1 py-3 text-xs font-black text-siakad-700 dark:text-siakad-300 bg-siakad-50 dark:bg-siakad-800 rounded-xl hover:bg-siakad-primary hover:text-white transition-all text-center">
                    {{ __('Lecturer Details') }}
                </a>
        ...
        <div class="card-saas p-10 text-center">
            <p class="text-siakad-400 font-bold">{{ __('No lecturer data available') }}</p>
        </div>
        @endforelse
    </div>
                ...
                <div class="relative">
                    ...
                    <form action="{{ route('admin.lecturer.store') }}" method="POST">
                        ...
                        <div class="space-y-5">
                            ...
                            <div>
                                <label class="block text-[10px] font-black text-siakad-400 uppercase tracking-widest mb-2 ml-1">{{ __('NIDN (ID Number)') }}</label>
                        ...
                        <div class="mt-8 flex items-center justify-end gap-3">
                            ...
                            <button type="submit" class="btn-primary-saas px-8 py-2.5 rounded-xl text-sm font-black shadow-lg shadow-siakad-600/20">{{ __('Save Lecturer') }}</button>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="relative">
                    ...
                    <form id="editForm" method="POST">
                        ...
                        <div class="mt-8 flex items-center justify-end gap-3">
                            ...
                            <button type="submit" class="btn-primary-saas px-8 py-2.5 rounded-xl text-sm font-black shadow-lg shadow-siakad-600/20">{{ __('Update Lecturer') }}</button>
                        </div>
                    </form>
                </div>
            </div>
