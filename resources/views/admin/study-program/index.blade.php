<x-app-layout>
    <div class="mb-10">
        <h1 class="text-3xl font-black tracking-tight text-siakad-900 dark:text-white">{{ __('Program Management') }}</h1>
    ...
    <div class="card-saas overflow-hidden mb-8" x-data="{ open: false }">
        ...
        <button @click="open = !open" type="button" class="w-full px-8 py-6 bg-siakad-50/50 dark:bg-siakad-900/20 border-b border-siakad-100/50 dark:border-siakad-800/50 flex items-center justify-between hover:bg-siakad-50 dark:hover:bg-siakad-900/40 transition-all duration-300 cursor-pointer text-start group">
            <div class="flex items-center gap-5">
                <div class="w-12 h-12 rounded-2xl bg-white dark:bg-siakad-800 flex items-center justify-center text-siakad-600 dark:text-siakad-400 text-lg font-black shadow-soft border border-siakad-100 dark:border-siakad-700 group-hover:scale-110 transition-transform">
                    {{ strtoupper(substr($f->name, 0, 1)) }}
                </div>
                <div class="text-start">
                    <h3 class="font-black text-siakad-900 dark:text-white text-base tracking-tight group-hover:text-siakad-primary transition-colors">{{ $f->name }}</h3>
                    <p class="text-[10px] text-siakad-400 font-black uppercase tracking-widest mt-0.5">{{ $f->studyPrograms->count() }} {{ __('Programs') }}</p>
                </div>
            </div>
            <div class="flex items-center gap-6">
                ...
                <!-- Chevron Icon -->
                <div class="p-2 rounded-xl bg-siakad-50 dark:bg-siakad-900/50 group-hover:bg-siakad-100 dark:group-hover:bg-siakad-900 transition-colors">
                ...
                <table class="w-full text-start border-collapse">
                    <thead>
                        <tr class="bg-siakad-50/30 dark:bg-siakad-900/10 border-b border-siakad-100/30 dark:border-siakad-800/30">
                            ...
                            <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start">{{ __('Program Name') }}</th>
                            <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center w-32">{{ __('Students') }}</th>
                            <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center w-32">{{ __('Lecturers') }}</th>
                    ...
                    <tbody class="divide-y divide-siakad-50/50 dark:divide-siakad-800/30">
                        ...
                        <tr class="hover:bg-siakad-50/20 dark:hover:bg-siakad-900/10 transition-colors group/row">
                            <td class="py-5 px-8 text-xs font-bold text-siakad-400">{{ $idx + 1 }}</td>
                            <td class="py-5 px-6">
                                <span class="text-sm font-black text-siakad-900 dark:text-white group-hover/row:text-siakad-primary transition-colors">{{ $p->name }}</span>
                            </td>
                            <td class="py-5 px-6 text-center">
                                <span class="inline-flex px-2.5 py-1 text-[10px] font-black bg-siakad-50 dark:bg-siakad-900 text-siakad-600 dark:text-siakad-400 rounded-lg border border-siakad-100 dark:border-siakad-800 uppercase">{{ $p->students_count ?? 0 }}</span>
                            </td>
                            <td class="py-5 px-6 text-center">
                                <span class="inline-flex px-2.5 py-1 text-[10px] font-black bg-siakad-50 dark:bg-siakad-900 text-siakad-600 dark:text-siakad-400 rounded-lg border border-siakad-100 dark:border-siakad-800 uppercase">{{ $p->lecturers_count ?? 0 }}</span>
                            </td>
                        ...
                        <tr>
                            <td colspan="5" class="py-12 text-center">
                                <p class="text-siakad-400 text-xs font-bold uppercase tracking-widest">{{ __('No study programs in this faculty yet') }}</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
    ...
    <div class="card-saas p-20 text-center flex flex-col items-center justify-center">
        <div class="w-20 h-20 bg-siakad-50 dark:bg-siakad-900/50 rounded-3xl flex items-center justify-center mb-6">
        ...
        <h3 class="text-xl font-black text-siakad-900 dark:text-white mb-2">{{ __('No faculties found') }}</h3>
        <p class="text-siakad-500 font-medium max-w-xs mx-auto mb-8">{{ __('Please add at least one faculty before creating study programs.') }}</p>
        <a href="{{ route('admin.faculty.index') }}" class="btn-primary-saas px-8 py-3 rounded-xl text-sm font-black shadow-lg shadow-siakad-600/20">
            ...
            <div class="relative bg-white dark:bg-siakad-900 rounded-[2rem] shadow-2xl w-full max-w-lg p-8 overflow-hidden animate-fade-in flex flex-col max-h-[90vh]">
                ...
                <form action="{{ route('admin.study-program.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1">{{ __('Faculty') }}</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1">{{ __('Program Name') }}</label>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-siakad-50 dark:border-siakad-800">
                        ...
                        <button type="submit" class="btn-primary-saas px-8 py-2.5 rounded-xl text-sm font-black shadow-lg shadow-siakad-primary/20">{{ __('Save Program') }}</button>
                    </div>
                </form>
            </div>
        </div>
