<x-app-layout>
    ...
    <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <p class="text-sm text-primary-secondary dark:text-gray-400">{{ __('Review and reply to reports sent by students') }}</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-900/50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-primary-secondary dark:text-gray-400 uppercase tracking-wider">{{ __('Student') }}</th>
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-primary-dark dark:text-white">{{ $report->user->name }}</div>
                            <div class="text-xs text-primary-secondary dark:text-gray-500">{{ $report->user->student->student_number ?? '-' }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-primary-dark dark:text-white">{{ Str::limit($report->subject, 40) }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <a href="{{ route('admin.report.show', $report) }}" class="text-primary-primary hover:text-primary-dark transition">
                                {{ __('Review') }}
                            </a>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5" class="px-6 py-8 text-center text-primary-secondary dark:text-gray-400">
                            {{ __('No reports found.') }}
                        </td>
                    </tr>
                    @endforelse
