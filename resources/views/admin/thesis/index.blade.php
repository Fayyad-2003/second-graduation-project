<x-app-layout>
    ...
    <div class="mb-6">
        <p class="text-sm text-siakad-secondary dark:text-gray-400">{{ __('Manage all student thesis submissions') }}</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="card-saas p-4 flex items-center gap-3 dark:bg-gray-800">
            ...
            <div>
                ...
                <p class="text-xs text-siakad-secondary dark:text-gray-400">{{ __('Total Thesis') }}</p>
            </div>
        </div>
        <div class="card-saas p-4 flex items-center gap-3 dark:bg-gray-800">
            ...
            <div>
                <p class="text-2xl font-bold text-siakad-dark dark:text-white">{{ $stats['active'] }}</p>
                <p class="text-xs text-siakad-secondary dark:text-gray-400">{{ __('Active') }}</p>
            </div>
        </div>
        <div class="card-saas p-4 flex items-center gap-3 dark:bg-gray-800">
            ...
            <div>
                <p class="text-2xl font-bold text-siakad-dark dark:text-white">{{ $stats['waiting_for_supervisor'] }}</p>
                <p class="text-xs text-siakad-secondary dark:text-gray-400">{{ __('Needs Advisor') }}</p>
            </div>
        </div>
        <div class="card-saas p-4 flex items-center gap-3 dark:bg-gray-800">
            ...
            <div>
                <p class="text-2xl font-bold text-siakad-dark dark:text-white">{{ $stats['completed'] }}</p>
                <p class="text-xs text-siakad-secondary dark:text-gray-400">{{ __('Finished') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="card-saas p-4 mb-6 dark:bg-gray-800">
        <form method="GET" class="flex flex-wrap items-end gap-4">
            <div class="flex-1 min-w-[200px]">
                <label class="block text-xs font-medium text-siakad-dark dark:text-gray-300 mb-1">{{ __('Search') }}</label>
            ...
            <div class="w-48">
                <label class="block text-xs font-medium text-siakad-dark dark:text-gray-300 mb-1">{{ __('Status') }}</label>
            ...
            <table class="w-full table-saas">
                <thead>
                    <tr class="bg-siakad-light/30 dark:bg-gray-900">
                        ...
                        <!-- Sortable: Title -->
                        <th class="text-left py-3 px-5 text-xs font-semibold text-siakad-secondary dark:text-gray-400 uppercase">
                            <a href="{{ request()->fullUrlWithQuery(['sort' => 'title', 'order' => request('sort') == 'title' && request('order') == 'asc' ? 'desc' : 'asc']) }}" class="group flex items-center gap-1 hover:text-siakad-primary transition">
                                {{ __('Title') }}
                                <span class="flex flex-col text-[10px] leading-none {{ request('sort') === 'title' ? 'text-siakad-primary' : 'text-gray-300' }}">
                        ...
                        <!-- Sortable: Advisor -->
                        <th class="text-left py-3 px-5 text-xs font-semibold text-siakad-secondary dark:text-gray-400 uppercase">
                            <a href="{{ request()->fullUrlWithQuery(['sort' => 'supervisor_name', 'order' => request('sort') == 'supervisor_name' && request('order') == 'asc' ? 'desc' : 'asc']) }}" class="group flex items-center gap-1 hover:text-siakad-primary transition">
                                {{ __('Advisor') }}
                                <span class="flex flex-col text-[10px] leading-none {{ in_array(request('sort'), ['supervisor_name', 'supervisor_name']) ? 'text-siakad-primary' : 'text-gray-300' }}">
                        ...
                        <th class="text-right py-3 px-5 text-xs font-semibold text-siakad-secondary dark:text-gray-400 uppercase">{{ __('Actions') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($thesisList as $index => $thesis)
                    <tr class="border-b border-siakad-light/50 dark:border-gray-700/50">
                        <td class="py-4 px-5 text-sm text-siakad-secondary dark:text-gray-400">{{ $thesisList->firstItem() + $index }}</td>
                        <td class="py-4 px-5">
                            <p class="font-medium text-siakad-dark dark:text-white">{{ $thesis->student->user->name }}</p>
                            <p class="text-xs text-siakad-secondary dark:text-gray-400">{{ $thesis->student->student_number }}</p>
                        </td>
                        <td class="py-4 px-5">
                            <p class="text-sm text-siakad-dark dark:text-white" title="{{ $thesis->title }}">{{ Str::limit($thesis->title, 50) }}</p>
                            @if($thesis->research_field)
                            <p class="text-xs text-siakad-secondary dark:text-gray-400">{{ $thesis->research_field }}</p>
                            @endif
                        </td>
                        <td class="py-4 px-5 text-sm">
                            @if($thesis->supervisor1)
                            <p class="text-siakad-dark dark:text-white">{{ $thesis->supervisor1->user->name }}</p>
                            @if($thesis->supervisor2)
                            <p class="text-xs text-siakad-secondary dark:text-gray-400">{{ $thesis->supervisor2->user->name }}</p>
                            @endif
                            @else
                        ...
                        <td class="py-4 px-5 text-right">
                            <a href="{{ route('admin.thesis.show', $thesis) }}" class="inline-flex items-center gap-1 text-sm text-siakad-primary dark:text-blue-400 hover:underline">{{ __('Details') }}</a>
                        </td>
                    </tr>
                    @empty
                    <tr class="dark:bg-gray-800">
                        <td colspan="6" class="py-12 text-center text-siakad-secondary dark:text-gray-400">{{ __('No thesis data available') }}</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="card-saas p-4 dark:bg-gray-800">
            <div class="flex items-start justify-between mb-3">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h4 class="font-bold text-siakad-dark dark:text-white">{{ $thesis->student->user->name }}</h4>
                        <p class="text-xs text-siakad-secondary dark:text-gray-400 font-mono">{{ $thesis->student->student_number }}</p>
                    </div>
                </div>
            ...
            <div class="mb-4">
                <p class="text-sm font-medium text-siakad-dark dark:text-white line-clamp-2 mb-1">{{ $thesis->title }}</p>
                @if($thesis->research_field)
                <p class="text-xs text-siakad-secondary dark:text-gray-400">{{ $thesis->research_field }}</p>
                @endif
            </div>
            ...
            <div class="bg-siakad-light/30 dark:bg-gray-700/30 rounded-lg p-3 mb-4">
                <p class="text-xs text-siakad-secondary dark:text-gray-400 mb-1">{{ __('Advisor') }}</p>
                @if($thesis->supervisor1)
                <p class="text-sm font-medium text-siakad-dark dark:text-white">{{ $thesis->supervisor1->user->name }}</p>
                @if($thesis->supervisor2)
                <p class="text-xs text-siakad-secondary dark:text-gray-400">{{ $thesis->supervisor2->user->name }}</p>
                @endif
                @else
            ...
            <a href="{{ route('admin.thesis.show', $thesis) }}" class="flex items-center justify-center w-full py-2 bg-siakad-light dark:bg-gray-700 text-siakad-dark dark:text-white font-medium rounded-lg hover:bg-gray-200 transition text-sm">
                {{ __('View Details') }}
            </a>
        </div>
        @empty
        <div class="card-saas p-8 text-center">
            <p class="text-siakad-secondary dark:text-gray-400">{{ __('No thesis data available') }}</p>
        </div>
        @endforelse
    </div>
