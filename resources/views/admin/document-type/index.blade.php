<x-app-layout>
    ...
    <div class="card-saas overflow-hidden">
        <table class="w-full table-saas">
            <thead>
                <tr class="bg-siakad-light/30">
                    <th class="text-left py-3 px-5 text-xs font-semibold text-siakad-secondary uppercase tracking-wider">{{ __('Name') }}</th>
                    <th class="text-left py-3 px-5 text-xs font-semibold text-siakad-secondary uppercase tracking-wider">{{ __('Required Files') }}</th>
                    <th class="text-center py-3 px-5 text-xs font-semibold text-siakad-secondary uppercase tracking-wider">{{ __('Status') }}</th>
            ...
            <tbody class="divide-y divide-siakad-light/50">
                ...
                <tr>
                    <td class="py-4 px-5">
                        <div class="font-bold text-siakad-dark">{{ $type->name }}</div>
                        <div class="text-xs text-siakad-secondary">{{ $type->description }}</div>
                    </td>
                    <td class="py-4 px-5">
                        <div class="flex flex-wrap gap-1">
                            @foreach($type->required_files as $file)
                            <span class="px-2 py-0.5 bg-siakad-light text-siakad-secondary text-[10px] rounded-full border border-siakad-light/50">
                                {{ $file['name'] }}
                            </span>
                            @endforeach
                        </div>
        ...
        <div class="px-5 py-4 border-t border-siakad-light/50">
            {{ $documentTypes->links() }}
        </div>
        @endif
    </div>
    ...
    <div id="createModal" class="fixed inset-0 z-50 overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen px-4">
            <div class="fixed inset-0 bg-siakad-dark/50 transition-opacity" onclick="this.parentElement.parentElement.classList.add('hidden')"></div>
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl max-w-lg w-full p-6">
                <h3 class="text-lg font-bold text-siakad-dark dark:text-white mb-4">{{ __('Add Document Type') }}</h3>
                <form action="{{ route('admin.document-type.store') }}" method="POST">
                    @csrf
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-siakad-dark mb-1">{{ __('Document Name') }}</label>
                            <input type="text" name="name" class="input-saas w-full" placeholder="{{ __('e.g., College ID Card') }}" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-siakad-dark mb-1">{{ __('Description') }}</label>
                            <textarea name="description" class="input-saas w-full" rows="2"></textarea>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-siakad-dark mb-2">{{ __('Required Files') }}</label>
                            ...
                            <button type="button" onclick="addFileRow('file-list-create')" class="text-xs font-bold text-siakad-primary hover:underline flex items-center gap-1">
                    ...
                    <div class="mt-6 flex justify-end gap-3">
                        <button type="button" onclick="this.closest('#createModal').classList.add('hidden')" class="px-4 py-2 text-sm font-medium text-siakad-secondary">{{ __('Cancel') }}</button>
                        <button type="submit" class="btn-primary-saas px-6 py-2 rounded-lg text-sm font-bold shadow-lg shadow-siakad-primary/20">{{ __('Create') }}</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div id="editModal" class="fixed inset-0 z-50 overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen px-4">
            <div class="fixed inset-0 bg-siakad-dark/50 transition-opacity" onclick="this.parentElement.parentElement.classList.add('hidden')"></div>
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl max-w-lg w-full p-6">
                <h3 class="text-lg font-bold text-siakad-dark dark:text-white mb-4">{{ __('Edit Document Type') }}</h3>
                <form id="editForm" method="POST">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <button type="button" onclick="addFileRow('file-list-edit')" class="text-xs font-bold text-siakad-primary hover:underline flex items-center gap-1">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                                {{ __('Add Another File') }}
                            </button>
                        </div>
                        <div class="flex items-center gap-2">
                            <input type="checkbox" name="is_active" id="editIsActive" value="1" class="rounded text-siakad-primary focus:ring-siakad-primary">
                            <label for="editIsActive" class="text-sm text-siakad-dark">{{ __('Active') }}</label>
                        </div>
                    </div>
                    <div class="mt-6 flex justify-end gap-3">
                        <button type="button" onclick="this.closest('#editModal').classList.add('hidden')" class="px-4 py-2 text-sm font-medium text-siakad-secondary">{{ __('Cancel') }}</button>
                        <button type="submit" class="btn-primary-saas px-6 py-2 rounded-lg text-sm font-bold shadow-lg shadow-siakad-primary/20">{{ __('Save Changes') }}</button>
                    </div>
                </form>
            </div>
        </div>
