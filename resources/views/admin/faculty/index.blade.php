<x-app-layout>
    <div class="mb-10">
        <h1 class="text-3xl font-black tracking-tight text-primary-900 dark:text-white">{{ __('Faculty Management') }}</h1>
            ...
            <table class="w-full text-start border-collapse">
                <thead>
                    <tr class="bg-primary-50/50 dark:bg-primary-900/30 border-b border-primary-100/50 dark:border-primary-800/50">
                        ...
                        <th class="py-5 px-6 text-[10px] font-black text-primary-400 uppercase tracking-widest text-start">{{ __('Faculty Name') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-primary-400 uppercase tracking-widest text-center">{{ __('Programs') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-primary-400 uppercase tracking-widest text-center">{{ __('Total Students') }}</th>
                ...
                <tbody class="divide-y divide-primary-50 dark:divide-primary-800/50">
                    ...
                    <tr class="hover:bg-primary-50/30 dark:hover:bg-primary-900/20 transition-colors group">
                        <td class="py-5 px-8 text-xs font-bold text-primary-400">{{ $index + 1 }}</td>
                        <td class="py-5 px-6">
                            <div class="flex items-center gap-4">
                                ...
                                <span class="text-sm font-black text-primary-900 dark:text-white">{{ $f->name }}</span>
                            </div>
                        </td>
                        <td class="py-5 px-6 text-center">
                            <span class="inline-flex px-2.5 py-1 text-[10px] font-black bg-primary-50 dark:bg-primary-900 text-primary-600 dark:text-primary-400 rounded-lg border border-primary-100 dark:border-primary-800 uppercase">{{ $f->study_programs_count ?? $f->studyPrograms->count() }}</span>
                        </td>
                        <td class="py-5 px-6 text-center">
                        ...
                        <td class="py-5 px-8 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.faculty.requirements', $f) }}" class="p-2 text-primary-secondary hover:text-primary-primary hover:bg-primary-primary/10 rounded-lg transition-colors" title="{{ __('Requirements') }}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                    </svg>
                                </a>
                                <button onclick="editFaculty({{ $f->id }}, '{{ $f->name }}')" class="p-2 text-primary-secondary hover:text-primary-primary hover:bg-primary-primary/10 rounded-lg transition-colors" title="{{ __('Edit') }}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <tr>
                        <td colspan="5" class="py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-primary-400 text-sm font-bold">{{ __('No faculty data available yet.') }}</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
    ...
    <div class="md:hidden space-y-4">
        ...
        <div class="card-saas p-4 dark:bg-gray-800">
            <div class="flex items-start justify-between mb-3">
                <h4 class="font-bold text-primary-dark dark:text-white">{{ $f->name }}</h4>
            </div>

            <div class="grid grid-cols-2 gap-3 mb-4">
                <div class="bg-gray-50 dark:bg-gray-700/50 p-2 rounded-lg text-center">
                    <span class="block text-[10px] text-primary-secondary dark:text-gray-400 uppercase tracking-wider">{{ __('Study Programs') }}</span>
                    <span class="font-bold text-primary-primary dark:text-blue-400">{{ $f->study_programs_count ?? $f->studyPrograms->count() }}</span>
                </div>
                <div class="bg-gray-50 dark:bg-gray-700/50 p-2 rounded-lg text-center">
                    <span class="block text-[10px] text-primary-secondary dark:text-gray-400 uppercase tracking-wider">{{ __('Students') }}</span>
                    <span class="font-bold text-primary-dark dark:text-white">{{ $f->studyPrograms->sum(fn($p) => $p->students->count()) }}</span>
                </div>
            </div>

            <div class="flex items-center gap-2 pt-3 border-t border-primary-light dark:border-gray-700">
                <a href="{{ route('admin.faculty.requirements', $f) }}" class="flex-1 py-2 text-sm font-medium text-primary-primary bg-primary-primary/10 rounded-lg hover:bg-primary-primary hover:text-white transition text-center">
                    {{ __('Reqs') }}
                </a>
                <button onclick="editFaculty({{ $f->id }}, '{{ $f->name }}')" class="flex-1 py-2 text-sm font-medium text-primary-secondary bg-primary-light/50 dark:bg-gray-700 dark:text-gray-300 rounded-lg hover:bg-primary-light hover:text-primary-primary dark:hover:bg-gray-600 transition text-center">
                    {{ __('Edit') }}
                </button>
        ...
        <div class="card-saas p-8 text-center">
            <p class="text-primary-secondary dark:text-gray-400 mb-2">{{ __('No faculty data available yet') }}</p>
        </div>
        @endforelse
    </div>
                ...
                <form action="{{ route('admin.faculty.store') }}" method="POST" class="flex flex-col min-h-0 space-y-6">
                    ...
                    <div class="overflow-y-auto pr-2 scrollbar-hide space-y-6 flex-1">
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black text-primary-400 uppercase tracking-widest ml-1">{{ __('Faculty Name') }} <span class="text-red-500">*</span></label>
                        ...
                        <div class="pt-4 border-t border-primary-50 dark:border-primary-800">
                            <p class="text-[10px] font-black text-primary-primary uppercase tracking-widest mb-4">{{ __('Graduation Requirements') }}</p>
                            <div class="space-y-1.5">
                                <label class="text-[10px] font-black text-primary-400 uppercase tracking-widest ml-1">{{ __('Total Required Credits') }} <span class="text-red-500">*</span></label>
                                <div class="relative">
                                    <input type="number" name="total_credits" min="0" value="0" class="input-saas w-full pr-16 font-black text-primary-primary" required>
                                    <div class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none">
                                        <span class="text-[10px] font-black text-primary-400 uppercase tracking-widest">{{ __('Credits') }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="pt-4 border-t border-primary-50 dark:border-primary-800">
                            ...
                            <div class="grid grid-cols-1 gap-3">
                                ...
                                <div class="flex items-center justify-between p-4 bg-primary-50/50 dark:bg-primary-800/50 rounded-2xl border border-primary-100/50 dark:border-primary-700/50">
                                    <span class="text-xs font-bold text-primary-700 dark:text-primary-300">{{ __($classification->name) }}</span>
                                    <div class="relative w-24">
                                        ...
                                        <div class="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
                                            <span class="text-[8px] font-black text-primary-400 uppercase tracking-widest">SKS</span>
                                        </div>
                                    </div>
                                </div>
                                @endforeach
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-primary-50 dark:border-primary-800 sticky bottom-0 bg-white dark:bg-primary-900">
                        ...
                        <button type="submit" class="btn-primary-saas px-8 py-2.5 rounded-xl text-sm font-black shadow-lg shadow-primary-primary/20">{{ __('Save Faculty') }}</button>
                    </div>
                </form>
            </div>
        </div>
