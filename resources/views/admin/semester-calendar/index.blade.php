<x-app-layout>
    <div class="mb-10 flex flex-col md:flex-row md:items-end md:justify-between gap-6">
        <div>
            <h1 class="text-3xl font-black tracking-tight text-siakad-900 dark:text-white">{{ __('Semester Calendar') }}</h1>
            ...
            <table class="w-full text-start border-collapse">
                <thead>
                    <tr class="bg-siakad-50/50 dark:bg-siakad-900/30 border-b border-siakad-100/50 dark:border-siakad-800/50">
                        <th class="py-5 px-8 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start w-32">{{ __('Week') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start w-40">{{ __('Date') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start">{{ __('Event Details') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center w-32">{{ __('Type') }}</th>
                ...
                <tbody class="divide-y divide-siakad-50 dark:divide-siakad-800/50">
                    ...
                    <tr class="hover:bg-siakad-50/30 dark:hover:bg-siakad-900/20 transition-colors group">
                        <td class="py-5 px-8 text-start">
                            <span class="inline-flex px-3 py-1 text-xs font-black bg-siakad-50 dark:bg-siakad-900 text-siakad-primary rounded-lg border border-siakad-100 dark:border-siakad-800 tracking-wider">
                                {{ $calendar->week_number ? __('Week') . ' ' . $calendar->week_number : '—' }}
                            </span>
                        </td>
                        <td class="py-5 px-6 text-start">
                            <div class="flex items-center gap-2.5">
                                <div class="p-2 rounded-xl bg-siakad-50 dark:bg-siakad-900 text-siakad-400 group-hover:text-siakad-primary transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                    </svg>
                                </div>
                                <span class="text-xs font-black text-siakad-900 dark:text-white font-mono uppercase tracking-widest">
                        ...
                        <td class="py-5 px-6 text-start">
                            <div class="min-w-0">
                                <p class="text-sm font-black text-siakad-900 dark:text-white leading-tight">{{ $calendar->title }}</p>
                                @if($calendar->description)
                                <p class="text-[10px] text-siakad-400 font-bold mt-1 line-clamp-1">{{ $calendar->description }}</p>
                                @endif
                            </div>
                        </td>
                        <td class="py-5 px-6 text-center">
                            ...
                            'other' => 'bg-siakad-50 text-siakad-600 dark:bg-siakad-950/30 dark:text-siakad-400 border-siakad-100 dark:border-siakad-900/50',
                            ];
                    ...
                    <tr>
                        <td colspan="5" class="py-16 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-siakad-400 font-bold uppercase tracking-widest text-[10px]">{{ __('No calendar entries found') }}</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
                ...
                <form action="{{ route('admin.semester-calendar.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1">{{ __('Week Number') }}</label>
                            <input type="number" name="week_number" class="input-saas w-full" placeholder="e.g. 1">
                        </div>
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1">{{ __('Date') }}</label>
                            <input type="date" name="date" class="input-saas w-full">
                        </div>
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1">{{ __('Title') }} <span class="text-red-500">*</span></label>
                        <input type="text" name="title" required class="input-saas w-full" placeholder="{{ __('Event title') }}">
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1">{{ __('Description') }}</label>
                        <textarea name="description" rows="3" class="input-saas w-full" placeholder="{{ __('Optional description') }}"></textarea>
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1">{{ __('Type') }} <span class="text-red-500">*</span></label>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-siakad-50 dark:border-siakad-800">
                        ...
                        <button type="submit" class="btn-primary-saas px-8 py-2.5 rounded-xl text-sm font-black shadow-lg shadow-siakad-primary/20">{{ __('Save Event') }}</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="relative bg-white dark:bg-siakad-900 rounded-[2rem] shadow-2xl w-full max-w-lg p-8 overflow-hidden animate-fade-in flex flex-col">
                ...
                <form id="editForm" method="POST" class="space-y-6">
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-siakad-50 dark:border-siakad-800">
                        ...
                        <button type="submit" class="btn-primary-saas px-8 py-2.5 rounded-xl text-sm font-black shadow-lg shadow-siakad-primary/20">{{ __('Update Event') }}</button>
                    </div>
                </form>
            </div>
        </div>
