<x-app-layout>
    <div class="mb-10">
        <h1 class="text-3xl font-black tracking-tight text-siakad-900 dark:text-white">{{ __('Class Management') }}</h1>
    ...
    <div class="mb-8 card-saas p-6">
        <form method="GET" class="flex flex-wrap items-center gap-4">
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto">
                ...
                @if(request('search'))
                <a href="{{ route('admin.class.index') }}" class="btn-ghost-saas px-4 py-2.5 rounded-xl text-sm font-bold text-siakad-400 hover:text-siakad-600">
                    {{ __('Reset') }}
                </a>
                @endif
            </div>
            ...
            <button type="button" onclick="document.getElementById('createModal').classList.remove('hidden')"
                class="btn-primary-saas px-6 py-2.5 rounded-xl text-sm font-black flex items-center gap-2 shadow-lg shadow-siakad-600/20 ml-auto">
            ...
            <table class="w-full text-start border-collapse">
                <thead>
                    <tr class="bg-siakad-50/50 dark:bg-siakad-900/30 border-b border-siakad-100/50 dark:border-siakad-800/50">
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start w-16">#</th>
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start">
                            <a href="{{ route('admin.class.index', array_merge(request()->all(), ['sort' => 'class_name', 'order' => request('sort') == 'class_name' && request('order') == 'asc' ? 'desc' : 'asc'])) }}" class="group flex items-center gap-1.5 hover:text-siakad-primary transition-colors">
                                {{ __('Class') }}
                        ...
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start">
                            <a href="{{ route('admin.class.index', array_merge(request()->all(), ['sort' => 'course_name', 'order' => request('sort') == 'course_name' && request('order') == 'asc' ? 'desc' : 'asc'])) }}" class="group flex items-center gap-1.5 hover:text-siakad-primary transition-colors">
                                {{ __('Course') }}
                        ...
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start">
                            <a href="{{ route('admin.class.index', array_merge(request()->all(), ['sort' => 'Lecturer', 'order' => request('sort') == 'Lecturer' && request('order') == 'asc' ? 'desc' : 'asc'])) }}" class="group flex items-center gap-1.5 hover:text-siakad-primary transition-colors">
                                {{ __('Lecturer') }}
                        ...
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start">{{ __('Schedule') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start">
                            <a href="{{ route('admin.class.index', array_merge(request()->all(), ['sort' => 'capacity', 'order' => request('sort') == 'capacity' && request('order') == 'asc' ? 'desc' : 'asc'])) }}" class="group flex items-center gap-1.5 hover:text-siakad-primary transition-colors">
                                {{ __('Capacity') }}
                        ...
                        <th class="py-5 px-8 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-end w-32">{{ __('Actions') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-siakad-50 dark:divide-siakad-800/50">
                    ...
                    <tr class="hover:bg-siakad-50/30 dark:hover:bg-siakad-900/20 transition-colors group">
                        <td class="py-5 px-6 text-xs font-bold text-siakad-400">{{ $classes->firstItem() + $index }}</td>
                        <td class="py-5 px-6">
                            <span class="inline-flex px-3 py-1 text-xs font-black bg-siakad-50 dark:bg-siakad-900 text-siakad-primary rounded-lg border border-siakad-100 dark:border-siakad-800 tracking-wider group-hover:bg-siakad-primary group-hover:text-white transition-all">{{ $k->class_name }}</span>
                        </td>
                        <td class="py-5 px-6">
                            <div>
                                <p class="text-sm font-black text-siakad-900 dark:text-white">{{ $k->course->course_name ?? '-' }}</p>
                                <p class="text-[10px] text-siakad-400 font-bold uppercase tracking-wider mt-0.5">{{ $k->course->course_code ?? '' }}</p>
                            </div>
                        </td>
                        <td class="py-5 px-6">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-lg bg-siakad-50 dark:bg-siakad-900/50 flex items-center justify-center text-siakad-600 dark:text-siakad-400 text-xs font-black">
                                    {{ strtoupper(substr($k->lecturer->user->name ?? '-', 0, 1)) }}
                                </div>
                                <span class="text-sm font-bold text-siakad-700 dark:text-siakad-300">{{ $k->lecturer->user->name ?? '-' }}</span>
                            </div>
                        </td>
                        <td class="py-5 px-6">
                            @if($schedule)
                            <div class="flex flex-col gap-1">
                                <div class="flex items-center gap-1.5 text-xs font-black text-siakad-900 dark:text-white">
                                    <svg class="w-3.5 h-3.5 text-siakad-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                    </svg>
                                    {{ __($schedule->day) }}
                                </div>
                                <div class="flex items-center gap-1.5 text-[10px] font-bold text-siakad-400 uppercase">
                        ...
                        <td class="py-5 px-6">
                            <div class="flex items-center gap-2">
                                <div class="flex-1 h-1.5 bg-siakad-50 dark:bg-siakad-900 rounded-full overflow-hidden min-w-[60px]">
                                    <div class="h-full bg-siakad-primary rounded-full" style="width: 100%"></div>
                                </div>
                                <span class="text-xs font-black text-siakad-700 dark:text-siakad-300">{{ $k->capacity }}</span>
                            </div>
                        </td>
                        <td class="py-5 px-8 text-end">
                            <div class="flex items-center justify-end gap-2">
                                <button onclick="editClass({{ json_encode([
                                    ...
                                    'room' => $schedule?->room,
                                ]) }})" class="p-2 text-siakad-primary hover:bg-siakad-primary/10 rounded-lg transition" title="{{ __('Edit') }}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <tr>
                        <td colspan="7" class="py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-siakad-400 text-sm font-medium">{{ __('No class data available') }}</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
    ...
    <div class="md:hidden space-y-4 mb-6">
        ...
        <div class="card-saas p-6 group">
            <div class="flex items-start justify-between mb-4">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-siakad-50 to-siakad-100 dark:from-siakad-900/50 dark:to-siakad-800/50 flex items-center justify-center text-siakad-600 dark:text-siakad-400 text-sm font-black shadow-sm">
                        {{ strtoupper(substr($k->class_name ?? '-', 0, 1)) }}
                    </div>
                    <div class="min-w-0">
                        <h4 class="font-black text-siakad-900 dark:text-white truncate group-hover:text-siakad-primary transition-colors">{{ $k->course->course_name ?? '-' }}</h4>
                        <p class="text-[10px] text-siakad-400 font-black uppercase tracking-widest mt-1">{{ $k->course->course_code ?? '' }} <span class="mx-1">•</span> {{ __('Class') }} {{ $k->class_name }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3 mb-6">
                <div class="bg-siakad-50/50 dark:bg-siakad-900/50 p-3 rounded-2xl border border-siakad-100/50 dark:border-siakad-800/50 col-span-2">
                    <span class="block text-[10px] text-siakad-400 font-black uppercase tracking-widest mb-1">{{ __('Lecturer') }}</span>
                    <span class="text-xs font-black text-siakad-900 dark:text-white line-clamp-1">{{ $k->lecturer->user->name ?? '-' }}</span>
                </div>
                <div class="bg-siakad-50/50 dark:bg-siakad-900/50 p-3 rounded-2xl border border-siakad-100/50 dark:border-siakad-800/50">
                    <span class="block text-[10px] text-siakad-400 font-black uppercase tracking-widest mb-1">{{ __('Schedule') }}</span>
                    @if($schedule)
                    <span class="text-[10px] font-black text-siakad-900 dark:text-white">{{ __($schedule->day) }}, {{ \Carbon\Carbon::parse($schedule->start_time)->format('H:i') }}</span>
                    @else
                    <span class="text-[10px] font-black text-siakad-300">{{ __('Not set') }}</span>
                    @endif
                </div>
                <div class="bg-siakad-50/50 dark:bg-siakad-900/50 p-3 rounded-2xl border border-siakad-100/50 dark:border-siakad-800/50">
                    <span class="block text-[10px] text-siakad-400 font-black uppercase tracking-widest mb-1">{{ __('Capacity') }}</span>
                    <span class="text-xs font-black text-siakad-900 dark:text-white">{{ $k->capacity }}</span>
                </div>
            </div>
        ...
        <div class="card-saas p-10 text-center">
            <p class="text-siakad-400 font-bold">{{ __('No class data available') }}</p>
        </div>
        @endforelse
    </div>
                ...
                <form action="{{ route('admin.class.store') }}" method="POST" class="space-y-6 overflow-y-auto max-h-[70vh] pr-2 scrollbar-hide">
                    ...
                    <div class="grid grid-cols-2 gap-5">
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1">{{ __('Class Name') }}</label>
                            <input type="text" name="class_name" placeholder="e.g., A" required class="input-saas w-full">
                        </div>
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1">{{ __('Capacity') }}</label>
                            <input type="number" name="capacity" min="1" placeholder="40" required class="input-saas w-full">
                        </div>
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1">{{ __('Course') }}</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1">{{ __('Lecturer') }}</label>
                    ...
                    <div class="pt-4 border-t border-siakad-50 dark:border-siakad-800">
                        <p class="text-[10px] font-black text-siakad-primary uppercase tracking-widest mb-4">{{ __('Academic Schedule') }}</p>
                        <div class="grid grid-cols-2 gap-5">
                            <div class="space-y-1.5">
                                <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1">{{ __('Day') }}</label>
                            ...
                            <div class="space-y-1.5">
                                <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1">{{ __('Room') }}</label>
                                <input type="text" name="room" placeholder="e.g., LT-101" class="input-saas w-full">
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-5 mt-4">
                            <div class="space-y-1.5">
                                <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1">{{ __('Start Time') }}</label>
                                <input type="time" name="start_time" class="input-saas w-full">
                            </div>
                            <div class="space-y-1.5">
                                <label class="text-[10px] font-black text-siakad-400 uppercase tracking-widest ml-1">{{ __('End Time') }}</label>
                                <input type="time" name="end_time" class="input-saas w-full">
                            </div>
                        </div>
                    </div>

                    <div class="flex justify-end gap-3 pt-4 sticky bottom-0 bg-white dark:bg-siakad-900 py-2 border-t border-siakad-50 dark:border-siakad-800">
                        ...
                        <button type="submit" class="btn-primary-saas px-8 py-2.5 rounded-xl text-sm font-black shadow-lg shadow-siakad-primary/20">{{ __('Create Class') }}</button>
                    </div>
                </form>
            </div>
        </div>
