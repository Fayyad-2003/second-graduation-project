<x-app-layout>
    ...
    <div class="card-saas p-4 mb-6 dark:bg-gray-800">
        <form method="GET" class="flex flex-wrap items-end gap-4">
            <div class="flex-1 min-w-[200px]"><label class="block text-xs font-medium text-siakad-dark dark:text-gray-300 mb-1">{{ __('Search') }}</label><input type="text" name="search" value="{{ request('search') }}" class="input-saas w-full text-sm dark:bg-gray-900 dark:border-gray-700 dark:text-white" placeholder="{{ __('Name, Student ID, Company...') }}"></div>
            <div class="w-48"><label class="block text-xs font-medium text-siakad-dark dark:text-gray-300 mb-1">{{ __('Status') }}</label><select name="status" class="input-saas w-full text-sm dark:bg-gray-900 dark:border-gray-700 dark:text-white"><option value="">{{ __('All') }}</option>
@foreach($statusList as $k => $v)<option value="{{ $k }}" {{ request('status') === $k ? 'selected' : '' }}>{{ $v }}</option>@endforeach</select></div>
        ...
        <table class="w-full table-saas">
            <thead>
                <tr class="bg-siakad-light/30 dark:bg-gray-900">
                    ...
                    <!-- Sortable: Company -->
                    <th class="text-left py-3 px-5 text-xs font-semibold text-siakad-secondary dark:text-gray-400 uppercase">
                        <a href="{{ request()->fullUrlWithQuery(['sort' => 'company_name', 'order' => request('sort') == 'company_name' && request('order') == 'asc' ? 'desc' : 'asc']) }}" class="group flex items-center gap-1 hover:text-siakad-primary transition">
                            {{ __('Company') }}
                            <span class="flex flex-col text-[10px] leading-none {{ request('sort') === 'company_name' ? 'text-siakad-primary' : 'text-gray-300' }}">
            ...
            <tr class="border-b border-siakad-light/50 dark:border-gray-700/50">
                <td class="py-4 px-5"><p class="font-medium text-siakad-dark dark:text-white">{{ $internship->student->user->name }}</p><p class="text-xs text-siakad-secondary dark:text-gray-400">{{ $internship->student->student_number }}</p></td>
                <td class="py-4 px-5 text-sm text-siakad-dark dark:text-white">{{ $internship->company_name }}</td>
                <td class="py-4 px-5 text-sm">@if($internship->supervisor)<span class="text-siakad-dark dark:text-white">{{ $internship->supervisor->user->name }}</span>@else<span class="text-amber-600 dark:text-amber-400 text-xs">{{ __('Not Yet') }}</span>@endif</td>
                ...
                <td class="py-4 px-5 text-right"><a href="{{ route('admin.internship.show', $internship) }}" class="text-sm text-siakad-primary dark:text-blue-400 hover:underline">{{ __('Details') }}</a></td>
            </tr>
            @empty
            <tr><td colspan="5" class="py-12 text-center text-siakad-secondary dark:text-gray-400">{{ __('No data available') }}</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
        ...
        <div class="card-saas p-4 dark:bg-gray-800">
            <div class="flex items-start justify-between mb-3">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h4 class="font-bold text-siakad-dark dark:text-white">{{ $internship->student->user->name }}</h4>
                        <p class="text-xs text-siakad-secondary dark:text-gray-400 font-mono">{{ $internship->student->student_number }}</p>
                    </div>
                </div>
            ...
            <div class="mb-4">
                <p class="text-xs text-siakad-secondary dark:text-gray-400 mb-1">{{ __('Company') }}</p>
                <p class="text-sm font-medium text-siakad-dark dark:text-white">{{ $internship->company_name }}</p>
            </div>
            ...
            <div class="bg-siakad-light/30 dark:bg-gray-700/30 rounded-lg p-3 mb-4">
                ...
                <p class="text-sm font-medium text-siakad-dark dark:text-white">{{ $internship->supervisor->user->name }}</p>
                @else
                <span class="text-amber-600 dark:text-amber-400 text-xs">{{ __('Not Yet') }}</span>
                @endif
            </div>

            <a href="{{ route('admin.internship.show', $internship) }}" class="flex items-center justify-center w-full py-2 bg-siakad-light dark:bg-gray-700 text-siakad-dark dark:text-white font-medium rounded-lg hover:bg-gray-200 transition text-sm">
                {{ __('View Details') }}
            </a>
        </div>
        @empty
        <div class="card-saas p-8 text-center">
            <p class="text-siakad-secondary dark:text-gray-400">{{ __('No data available') }}</p>
        </div>
        @endforelse
    </div>
