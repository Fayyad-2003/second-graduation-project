        ...
        <div class="lg:col-span-1">
            <div class="card-saas p-6 sticky top-24 dark:bg-gray-800">
                <div class="text-center mb-6">
                    ...
                    <h3 class="text-xl font-bold text-siakad-dark dark:text-white">{{ $studyPlan->student->user->name ?? '-' }}</h3>
                    <p class="text-siakad-secondary dark:text-gray-400">{{ $studyPlan->student->student_number ?? '-' }}</p>
                </div>

                <div class="space-y-3 text-sm">
                    <div class="flex justify-between">
                        <span class="text-siakad-secondary dark:text-gray-400">{{ __('Study Program') }}</span>
                        <span class="font-medium text-siakad-dark dark:text-white text-right">{{ $studyPlan->student->studyProgram->name ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-siakad-secondary dark:text-gray-400">{{ __('Batch') }}</span>
                        <span class="font-medium text-siakad-dark dark:text-white">{{ $studyPlan->student->batch ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-siakad-secondary dark:text-gray-400">{{ __('Academic Year') }}</span>
                        <span class="font-medium text-siakad-dark dark:text-white">{{ $studyPlan->academicYear->year ?? '-' }} {{ $studyPlan->academicYear->semester ?? '' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-siakad-secondary dark:text-gray-400">{{ __('Total Credits') }}</span>
                        <span class="font-bold text-siakad-primary dark:text-blue-400 text-lg">{{ $totalCredits }}</span>
                    </div>
                </div>

                <div class="mt-6 pt-6 border-t border-siakad-light dark:border-gray-700">
                    <div class="flex items-center justify-between">
                        <span class="text-siakad-secondary dark:text-gray-400">{{ __('Status') }}</span>
                ...
                <a href="{{ url('admin/study-plan-approval') }}" class="mt-6 block text-center text-sm text-siakad-secondary dark:text-gray-400 hover:text-siakad-primary dark:hover:text-blue-400 transition">
                    ← {{ __('Back to List') }}
                </a>
            </div>
        </div>
        ...
        <div class="lg:col-span-2">
            <div class="card-saas overflow-hidden dark:bg-gray-800">
                <div class="p-6 border-b border-siakad-light dark:border-gray-700">
                    <h3 class="text-lg font-bold text-siakad-dark dark:text-white">{{ __('Courses Taken') }}</h3>
                    <p class="text-sm text-siakad-secondary dark:text-gray-400">{{ $studyPlan->details->count() }} {{ __('courses') }}</p>
                </div>
                ...
                <div class="divide-y divide-siakad-light dark:divide-gray-700">
                    ...
                    <div class="p-4 flex items-center gap-4 hover:bg-siakad-light/30 dark:hover:bg-gray-700/50 transition">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="font-semibold text-siakad-dark dark:text-white">{{ $detail->academicClass->course->course_name ?? '-' }}</p>
                            <p class="text-sm text-siakad-secondary dark:text-gray-400">{{ $detail->academicClass->course->course_code ?? '-' }} • {{ $detail->academicClass->lecturer->user->name ?? '-' }}</p>
                        </div>
                        <div class="text-center">
                            ...
                            <p class="text-xs text-siakad-secondary dark:text-gray-400 mt-1">{{ __('Credits') }}</p>
                        </div>
                    </div>
                    @empty
