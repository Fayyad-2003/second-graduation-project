<x-app-layout>
    <div class="mb-10">
        <h1 class="text-3xl font-black tracking-tight text-primary-900 dark:text-white">{{ __('User Management') }}</h1>
    ...
    <div class="mb-8 card-saas p-6">
        <div class="flex flex-wrap items-center justify-between gap-6">
            <form method="GET" class="flex flex-wrap items-center gap-4 flex-1">
                ...
                <button type="submit" class="btn-primary-saas px-6 py-2.5 rounded-xl text-sm font-black shadow-lg shadow-primary-600/20">{{ __('Filter') }}</button>
            </form>
            <button onclick="openModal('createModal')" class="btn-primary-saas px-6 py-2.5 rounded-xl text-sm font-black flex items-center gap-2 shadow-lg shadow-primary-600/20">
            ...
            <table class="w-full text-start border-collapse">
                <thead>
                    <tr class="bg-primary-50/50 dark:bg-primary-900/30 border-b border-primary-100/50 dark:border-primary-800/50">
                        <th class="py-5 px-8 text-[10px] font-black text-primary-400 uppercase tracking-widest text-start">{{ __('User Info') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-primary-400 uppercase tracking-widest text-start">{{ __('Email Address') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-primary-400 uppercase tracking-widest text-center">{{ __('Role') }}</th>
                        <th class="py-5 px-6 text-[10px] font-black text-primary-400 uppercase tracking-widest text-start">{{ __('Faculty Context') }}</th>
                ...
                <tbody class="divide-y divide-primary-50 dark:divide-primary-800/50">
                    ...
                    <tr class="hover:bg-primary-50/30 dark:hover:bg-primary-900/20 transition-colors group">
                        <td class="py-5 px-8">
                            <div class="flex items-center gap-4 text-start">
                                ...
                                <div class="min-w-0">
                                    <p class="text-sm font-black text-primary-900 dark:text-white truncate">{{ $user->name }}</p>
                                    @if($user->student)
                                    <p class="text-[10px] text-primary-400 font-bold uppercase tracking-wider text-start font-mono">{{ $user->student->student_number }}</p>
                                    @elseif($user->lecturer)
                                    <p class="text-[10px] text-primary-400 font-bold uppercase tracking-wider text-start font-mono">{{ $user->lecturer->lecturer_number }}</p>
                                    @endif
                                </div>
                            </div>
                        </td>
                        <td class="py-5 px-6 text-start">
                            <span class="text-sm font-medium text-primary-500">{{ $user->email }}</span>
                        </td>
                        <td class="py-5 px-6 text-center">
                            ...
                            $roleColor = $roleColors[$user->role] ?? 'bg-primary-50 text-primary-400 border-primary-100';
                            @endphp
                        ...
                        <td class="py-5 px-6 text-start">
                            <span class="text-xs font-bold text-primary-600 dark:text-primary-400">{{ $user->faculty?->name ?? ($user->student?->studyProgram?->faculty?->name ?? ($user->lecturer?->studyProgram?->faculty?->name ?? '-')) }}</span>
                        </td>
                        <td class="py-5 px-8 text-end">
                            <div class="flex items-center justify-end gap-2">
                                <button onclick="openEditModal({{ json_encode($user) }})" class="p-2 text-primary-secondary hover:text-primary-primary hover:bg-primary-primary/10 rounded-lg transition" title="{{ __('Edit') }}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="py-16 text-center text-primary-400 font-bold text-sm">{{ __('No users found.') }}</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="md:hidden space-y-4 mb-6">
        ...
        <div class="card-saas p-6 group">
            <div class="flex items-start justify-between mb-4">
                <div class="flex items-center gap-4">
                    ...
                    <div class="min-w-0">
                        <h4 class="font-black text-primary-900 dark:text-white truncate group-hover:text-primary-primary transition-colors">{{ $user->name }}</h4>
                        <p class="text-[10px] text-primary-400 font-black uppercase tracking-widest mt-1">{{ $user->email }}</p>
                    </div>
                </div>
                @php
                $roleColor = $roleColors[$user->role] ?? 'bg-primary-50 text-primary-400 border-primary-100';
            ...
            <div class="grid grid-cols-1 gap-3 mb-6">
                <div class="bg-primary-50/50 dark:bg-primary-900/50 p-3 rounded-2xl border border-primary-100/50 dark:border-primary-800/50">
                    <span class="block text-[10px] text-primary-400 font-black uppercase tracking-widest mb-1">{{ __('Faculty Context') }}</span>
                    <span class="text-xs font-black text-primary-900 dark:text-white line-clamp-1">{{ $user->faculty?->name ?? ($user->student?->studyProgram?->faculty?->name ?? ($user->lecturer?->studyProgram?->faculty?->name ?? '-')) }}</span>
                </div>
            </div>

            <div class="flex items-center gap-3 pt-4 border-t border-primary-50 dark:border-primary-800">
                <button onclick="openEditModal({{ json_encode($user) }})" class="flex-1 py-3 text-xs font-black text-primary-700 dark:text-primary-300 bg-primary-50 dark:bg-primary-800 rounded-xl hover:bg-primary-primary hover:text-white transition-all text-center">
                    {{ __('Edit User') }}
                </button>
                @if($user->id !== auth()->id())
        ...
        <div class="card-saas p-10 text-center">
            <p class="text-primary-400 font-bold">{{ __('No user data available') }}</p>
        </div>
        @endforelse
    </div>
                ...
                <div class="relative">
                    ...
                    <form action="{{ route('admin.users.store') }}" method="POST">
                        ...
                        <div class="space-y-5">
                            ...
                            <div>
                                <label class="block text-[10px] font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('System Role') }}</label>
                            ...
                            <div id="createFacultyField" class="hidden animate-fade-in">
                                <label class="block text-[10px] font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('Assign Faculty') }}</label>
                        ...
                        <div class="mt-8 flex items-center justify-end gap-3">
                            ...
                            <button type="submit" class="btn-primary-saas px-8 py-2.5 rounded-xl text-sm font-black shadow-lg shadow-primary-600/20">{{ __('Save User') }}</button>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="relative">
                    ...
                    <form id="editForm" method="POST">
                        ...
                        <div class="mt-8 flex items-center justify-end gap-3">
                            ...
                            <button type="submit" class="btn-primary-saas px-8 py-2.5 rounded-xl text-sm font-black shadow-lg shadow-primary-600/20">{{ __('Update User') }}</button>
                        </div>
                    </form>
                </div>
            </div>
