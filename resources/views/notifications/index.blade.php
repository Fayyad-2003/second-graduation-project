<x-app-layout>
    ...
    <div class="card-saas overflow-hidden">
        ...
        @endphp
        <div class="flex items-start gap-4 p-5 {{ !$isRead ? 'bg-indigo-50/30' : '' }}" style="border-bottom: 1px solid var(--border-color);">
            <div class="w-10 h-10 rounded-xl bg-{{ $notif->color ?? 'blue' }}-100 flex items-center justify-center text-lg">
                {{ $notif->icon ?? '🔔' }}
            </div>
            ...
            <form action="{{ route('notifications.mark-read', $notif->id) }}" method="POST">
                @csrf
                <button type="submit" class="p-2 rounded-lg hover:bg-slate-100 transition text-slate-400 hover:text-indigo-600" title="{{ __('Mark as read') }}">
