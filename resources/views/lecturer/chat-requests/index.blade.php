<x-app-layout>
    ...
    <div class="mb-6">
        <h2 class="text-xl font-bold text-primary-dark dark:text-white">{{ __('Manage Chat Requests') }}</h2>
        <p class="text-sm text-primary-secondary dark:text-gray-400">{{ __('Approve or reject chat requests from students') }}</p>
    </div>
    ...
    <div class="card-saas p-8 text-center dark:bg-gray-800">
        ...
        <p class="text-primary-secondary dark:text-gray-400">{{ __('No chat requests yet.') }}</p>
    </div>
    @else
        ...
        <div class="card-saas p-5 dark:bg-gray-800">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div class="flex items-start gap-4">
                    <div class="w-12 h-12 bg-primary-primary/10 rounded-full flex items-center justify-center text-primary-primary font-bold text-lg flex-shrink-0">
                        {{ strtoupper(substr($request->student->user->name, 0, 1)) }}
                    </div>
                    <div>
                        <h3 class="font-bold text-primary-dark dark:text-white">{{ $request->student->user->name }}</h3>
                        ...
                        <p class="text-xs text-primary-secondary dark:text-gray-500 mt-1">
                            {{ $request->created_at->diffForHumans() }}
                        </p>
                        @if($request->message)
                        <div class="mt-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg text-sm text-primary-dark dark:text-gray-300 italic">
                            "{{ $request->message }}"
                        </div>
                        @endif
                    </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route('chat.show', $request) }}" class="px-4 py-2 bg-primary-primary text-white text-sm font-semibold rounded-lg hover:bg-primary-primary/90 transition flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
