<table class="w-full text-start border-collapse">
    <thead>
        <tr class="bg-siakad-50/50 dark:bg-siakad-900/30 border-b border-siakad-100/50 dark:border-siakad-800/50">
            ...
            <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start sortable-header cursor-pointer hover:text-siakad-primary transition-colors" data-sort="name">
                <div class="flex items-center gap-2">
                    {{ __('Students') }}
                    <span class="sort-icon">
                        @if(request('sort') === 'name')
                        @if(request('dir') === 'asc')
                        <svg class="w-3.5 h-3.5 text-siakad-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start sortable-header cursor-pointer hover:text-siakad-primary transition-colors" data-sort="student_number">
                <div class="flex items-center gap-2">
                    {{ __('Student ID') }}
                    <span class="sort-icon">
            ...
            <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start sortable-header cursor-pointer hover:text-siakad-primary transition-colors" data-sort="batch">
                <div class="flex items-center gap-2">
                    {{ __('Batch') }}
                    <span class="sort-icon">
            ...
            <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start sortable-header cursor-pointer hover:text-siakad-primary transition-colors" data-sort="status">
                <div class="flex items-center gap-2">
                    {{ __('Status') }}
                    <span class="sort-icon">
    ...
    <tbody class="divide-y divide-siakad-50 dark:divide-siakad-800/50">
        ...
        <tr class="hover:bg-siakad-50/30 dark:hover:bg-siakad-900/20 transition-colors group">
            <td class="py-5 px-8 text-xs font-bold text-siakad-400 text-start">{{ $advisedStudents->firstItem() + $index }}</td>
            <td class="py-5 px-6 text-start">
                <span class="text-sm font-black text-siakad-900 dark:text-white group-hover:text-siakad-primary transition-colors">{{ $m->user->name ?? '-' }}</span>
            </td>
            <td class="py-5 px-6 text-start">
                <span class="text-xs font-black text-siakad-700 dark:text-siakad-300 font-mono tracking-wider bg-siakad-50 dark:bg-siakad-900 px-2 py-1 rounded-lg border border-siakad-100 dark:border-siakad-800">{{ $m->student_number }}</span>
            </td>
            <td class="py-5 px-6 text-start">
                <span class="inline-flex px-3 py-1 rounded-lg text-[10px] font-black bg-siakad-50 dark:bg-siakad-900 text-siakad-primary border border-siakad-100 dark:border-siakad-800 uppercase tracking-widest shadow-sm">{{ $m->batch }}</span>
            </td>
            <td class="py-5 px-6 text-start">
                @php
                ...
                default => 'bg-siakad-50 dark:bg-siakad-900/50 text-siakad-400 border-siakad-100 dark:border-siakad-800',
                };
        ...
        <tr>
            <td colspan="5" class="py-16 text-center">
                <div class="flex flex-col items-center">
                    ...
                    <p class="text-siakad-400 font-bold text-sm">{{ __('No supervised students found.') }}</p>
                </div>
            </td>
        </tr>
        @endforelse
