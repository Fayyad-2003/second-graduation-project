<x-app-layout>
    <div class="mb-10">
        <h1 class="text-3xl font-black tracking-tight text-siakad-900 dark:text-white">{{ __('Internship Supervision') }}</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="card-saas p-6 flex items-center gap-4 relative overflow-hidden group hover:-translate-y-1 transition-all duration-300">
            ...
            <div>
                <p class="text-3xl font-black text-siakad-900 dark:text-white leading-none">{{ $internshipList->count() }}</p>
                <p class="text-[10px] text-siakad-400 font-black uppercase tracking-widest mt-2">{{ __('Total Guided') }}</p>
            </div>
        </div>
        <div class="card-saas p-6 flex items-center gap-4 relative overflow-hidden group hover:-translate-y-1 transition-all duration-300">
            ...
            <div>
                <p class="text-3xl font-black text-siakad-900 dark:text-white leading-none">{{ $internshipList->where('status', 'ongoing')->count() }}</p>
                <p class="text-[10px] text-siakad-400 font-black uppercase tracking-widest mt-2">{{ __('In Progress') }}</p>
            </div>
        </div>
        <div class="card-saas p-6 flex items-center gap-4 relative overflow-hidden group hover:-translate-y-1 transition-all duration-300">
            ...
            <div>
                <p class="text-3xl font-black text-siakad-900 dark:text-white leading-none">{{ $internshipList->where('status', 'completed')->count() }}</p>
                <p class="text-[10px] text-siakad-400 font-black uppercase tracking-widest mt-2">{{ __('Finished') }}</p>
            </div>
        </div>
    </div>
            ...
            <table class="w-full text-start border-collapse" id="kpTable">
                <thead>
                    <tr class="bg-siakad-50/50 dark:bg-siakad-900/30 border-b border-siakad-100/50 dark:border-siakad-800/50">
                        ...
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start sortable-header cursor-pointer hover:text-siakad-primary transition-colors" data-sort="perusahaan" onclick="sortTable('perusahaan')">
                            <div class="flex items-center gap-2">
                                {{ __('Company') }}
                                <svg class="w-3.5 h-3.5 opacity-30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4"></path>
                        ...
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center">{{ __('Period') }}</th>
                ...
                <tbody id="tableBody" class="divide-y divide-siakad-50 dark:divide-siakad-800/50">
                    ...
                    <tr class="kp-row hover:bg-siakad-50/30 dark:hover:bg-siakad-900/20 transition-colors group"
                        ...
                        <td class="py-5 px-6 text-start">
                            <p class="text-sm font-black text-siakad-900 dark:text-white group-hover:text-siakad-primary transition-colors">{{ $internship->student->user->name }}</p>
                            <p class="text-[10px] text-siakad-400 font-black uppercase tracking-widest mt-1">{{ $internship->student->student_number }}</p>
                        </td>
                        <td class="py-5 px-6 text-start">
                            <span class="text-sm font-black text-siakad-700 dark:text-siakad-300">{{ $internship->company_name }}</span>
                        </td>
                        <td class="py-5 px-6 text-center">
                            <span class="inline-flex px-3 py-1 rounded-lg text-[10px] font-black bg-siakad-50 dark:bg-siakad-900 text-siakad-400 border border-siakad-100 dark:border-siakad-800 uppercase tracking-widest">{{ $internship->start_date->format('d/m') }} - {{ $internship->completion_date->format('d/m/Y') }}</span>
                        </td>
                        <td class="py-5 px-6 text-center">
                            @php
                        ...
                        <td class="py-5 px-8 text-end">
                            <a href="{{ route('lecturers.internship.show', $internship) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-siakad-900 dark:bg-siakad-800 text-white text-xs font-black rounded-xl hover:bg-siakad-primary transition-all shadow-lg shadow-siakad-900/10">
                                {{ __('Details') }}
                        ...
                        <td colspan="6" class="py-16 text-center">
                            <div class="flex flex-col items-center">
                                <div class="w-16 h-16 bg-siakad-50 dark:bg-siakad-900/50 rounded-2xl flex items-center justify-center mb-4">
                                    <svg class="w-8 h-8 text-siakad-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                                </div>
                                <p class="text-siakad-400 font-bold text-sm">{{ __('No data available') }}</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
