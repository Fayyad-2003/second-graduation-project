<x-app-layout>
    <x-slot name="header">
        {{ __('Edit Question') }} - {{ $course->course_name }}
    </x-slot>
        ...
        <div class="card-saas p-8 relative overflow-hidden">
            ...
            <div class="relative">
                <div class="flex items-center justify-between mb-8">
                    ...
                    <a href="{{ route('lecturers.exam-questions.index', $class->id) }}" class="text-primary-400 hover:text-primary-600 transition-colors">
                ...
                <form action="{{ route('lecturers.exam-questions.update', [$class->id, $question->id]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="space-y-5">
                        <div>
                            <label class="block text-xs font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('Question Type') }}</label>
                            <select name="question_type" id="question-type" class="input-saas w-full px-4 py-3 text-sm rounded-xl" required>
                                ...
                                <option value="short_answer" {{ $question->question_type === 'short_answer' ? 'selected' : '' }}>{{ __('Short Answer') }}</option>
                                <option value="essay" {{ $question->question_type === 'essay' ? 'selected' : '' }}>{{ __('Essay') }}</option>
                            </select>
                        </div>

                        <div>
                            <label class="block text-xs font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('Question Text') }}</label>
                            <textarea name="question_text" id="question-text" rows="4" class="input-saas w-full px-4 py-3 text-sm rounded-xl" placeholder="{{ __('Enter your question here...') }}" required>{{ $question->question_text }}</textarea>
                        </div>

                        <div id="options-container" class="hidden">
                            <label class="block text-xs font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('Options (for Multiple Choice)') }}</label>
                            <div id="options-list" class="space-y-3">
                                @if($question->options && is_array($question->options))
                                @foreach($question->options as $index => $option)
                                <div class="flex gap-2 items-center">
                                    <span class="font-bold text-primary-500 w-6">{{ chr(65 + $index) }}</span>
                                    <input type="text" name="options[]" class="input-saas flex-1 px-4 py-2 text-sm rounded-xl" value="{{ $option }}">
                                </div>
                                @endforeach
                                @else
                                <div class="flex gap-2 items-center">
                                    <span class="font-bold text-primary-500 w-6">A</span>
                                    <input type="text" name="options[]" class="input-saas flex-1 px-4 py-2 text-sm rounded-xl" placeholder="{{ __('Option A') }}">
                                </div>
                                <div class="flex gap-2 items-center">
                                    <span class="font-bold text-primary-500 w-6">B</span>
                                    <input type="text" name="options[]" class="input-saas flex-1 px-4 py-2 text-sm rounded-xl" placeholder="{{ __('Option B') }}">
                                </div>
                                <div class="flex gap-2 items-center">
                                    <span class="font-bold text-primary-500 w-6">C</span>
                                    <input type="text" name="options[]" class="input-saas flex-1 px-4 py-2 text-sm rounded-xl" placeholder="{{ __('Option C') }}">
                                </div>
                                <div class="flex gap-2 items-center">
                                    <span class="font-bold text-primary-500 w-6">D</span>
                        ...
                        <div>
                            <label class="block text-xs font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('Correct Answer') }}</label>
                        ...
                        <div>
                            <label class="block text-xs font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('Difficulty') }}</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('Points') }}</label>
                                <input type="number" name="points" step="0.01" min="0" class="input-saas w-full px-4 py-3 text-sm rounded-xl" value="{{ $question->points }}" required>
                            </div>
                            <div>
                                <label class="block text-xs font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('Order') }}</label>
                                <input type="number" name="order" min="0" class="input-saas w-full px-4 py-3 text-sm rounded-xl" value="{{ $question->order }}" required>
                            </div>
                        </div>
                    </div>
