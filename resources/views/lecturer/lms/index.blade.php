<x-app-layout>
    ...
    <div class="mb-6">
        <h2 class="text-xl font-bold text-siakad-dark dark:text-white">{{ __('Select Class') }}</h2>
        <p class="text-sm text-siakad-secondary dark:text-gray-400">{{ __('Manage materials and assignments for the classes you teach') }}</p>
    </div>
    ...
    <div class="card-saas p-8 text-center dark:bg-gray-800">
        ...
        <p class="text-siakad-secondary dark:text-gray-400">{{ __('You are not teaching any classes.') }}</p>
    </div>
    @else
        ...
        <div class="class-card card-saas dark:bg-gray-800 overflow-hidden hover:shadow-lg transition-shadow" data-search="{{ strtolower($class->course->course_name . ' ' . $class->course->course_code . ' ' . $class->class_name) }}">
            <div class="p-5">
                <div class="flex items-start gap-3 mb-4">
                    ...
                    <div class="min-w-0">
                        <h3 class="font-semibold text-siakad-dark dark:text-white truncate">{{ $class->course->course_name }}</h3>
                        <p class="text-xs text-siakad-secondary dark:text-gray-400">{{ $class->course->course_code }} • {{ $class->course->credits }} {{ __('Credits') }}</p>
                    </div>
                </div>
                ...
                <div class="flex gap-2">
                    <a href="{{ route('lecturers.material.index', $class->id) }}" class="flex-1 text-center px-3 py-2 text-sm font-medium bg-siakad-light dark:bg-gray-700 text-siakad-dark dark:text-white rounded-lg hover:bg-siakad-light/80 dark:hover:bg-gray-600 transition">
