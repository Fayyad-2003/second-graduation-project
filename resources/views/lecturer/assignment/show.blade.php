<x-app-layout>
    ...
    <!-- Back Link -->
    <div class="mb-4">
        <a href="{{ route('lecturers.assignment.index', $class->id) }}" class="text-sm text-primary-secondary hover:text-primary-primary transition flex items-center gap-1">
    ...
    <div class="card-saas p-5 mb-6 dark:bg-gray-800">
        <div class="flex items-start justify-between mb-4">
            <div>
                ...
                <p class="text-sm text-primary-secondary dark:text-gray-400">{{ $class->course->course_name }} - {{ $class->class_name }}</p>
            </div>
        ...
        <p class="text-primary-secondary dark:text-gray-400 mb-4">{{ $assignment->description }}</p>
        @endif
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
        <div class="card-saas p-4 dark:bg-gray-800 text-center">
            <p class="text-2xl font-bold text-primary-primary">{{ $enrolledStudents->count() }}</p>
            <p class="text-xs text-primary-secondary">{{ __('Total Students') }}</p>
        </div>
        <div class="card-saas p-4 dark:bg-gray-800 text-center">
            <p class="text-2xl font-bold text-green-500">{{ $assignment->submissions->count() }}</p>
            <p class="text-xs text-primary-secondary">{{ __('Submitted') }}</p>
        </div>
        <div class="card-saas p-4 dark:bg-gray-800 text-center">
            <p class="text-2xl font-bold text-blue-500">{{ $assignment->submissions->whereNotNull('grade')->count() }}</p>
            <p class="text-xs text-primary-secondary">{{ __('Graded') }}</p>
        </div>
    </div>
    ...
    <div class="card-saas overflow-hidden dark:bg-gray-800">
        <div class="px-5 py-4 border-b border-primary-light dark:border-gray-700">
            <h3 class="font-semibold text-primary-dark dark:text-white">{{ __('Submission List') }}</h3>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-gray-50 dark:bg-gray-700/50">
                    <tr>
                        <th class="py-3 px-4 text-left text-xs font-medium text-primary-secondary dark:text-gray-400 uppercase">{{ __('Student') }}</th>
                        <th class="py-3 px-4 text-left text-xs font-medium text-primary-secondary dark:text-gray-400 uppercase">{{ __('Student ID') }}</th>
                        <th class="py-3 px-4 text-left text-xs font-medium text-primary-secondary dark:text-gray-400 uppercase">{{ __('Status') }}</th>
                        <th class="py-3 px-4 text-left text-xs font-medium text-primary-secondary dark:text-gray-400 uppercase">{{ __('Submit Time') }}</th>
                        <th class="py-3 px-4 text-left text-xs font-medium text-primary-secondary dark:text-gray-400 uppercase">{{ __('Grade') }}</th>
                        <th class="py-3 px-4 text-right text-xs font-medium text-primary-secondary dark:text-gray-400 uppercase">{{ __('Actions') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-primary-light dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                        <td class="py-3 px-4 text-sm text-primary-dark dark:text-white">{{ $student->user->name }}</td>
                        <td class="py-3 px-4 text-sm text-primary-secondary dark:text-gray-400 font-mono">{{ $student->student_number }}</td>
                        <td class="py-3 px-4">
                            @if($submission)
                        ...
                        <td class="py-3 px-4 text-sm text-primary-secondary dark:text-gray-400">
                            {{ $submission?->submitted_at?->format('d M Y, H:i') ?? '-' }}
                        </td>
                        <td class="py-3 px-4">
                            @if($submission && $submission->isGraded())
                            <span class="font-bold text-primary-dark dark:text-white">{{ $submission->grade }}</span>
                            <span class="text-xs text-primary-secondary">({{ $submission->grade_letter }})</span>
                            @elseif($submission)
                            <span class="text-primary-secondary">{{ __('Not Graded') }}</span>
                            @else
                            <span class="text-primary-secondary">-</span>
                            @endif
                        </td>
                        <td class="py-3 px-4 text-right">
                            @if($submission)
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('lecturers.assignment.submission.download', [$class->id, $submission->id]) }}" class="p-1.5 text-primary-secondary hover:text-primary-primary hover:bg-primary-primary/10 rounded transition" title="{{ __('Download') }}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path>
                                    </svg>
                                </a>
                                <button onclick="openGradeModal({{ $submission->id }}, '{{ $student->user->name }}', {{ $submission->grade ?? 'null' }}, '{{ addslashes($submission->feedback ?? '') }}')" class="px-2 py-1 text-xs bg-primary-primary text-white rounded hover:bg-primary-primary/90 transition">
    ...
    <div id="gradeModal" class="hidden fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
        <div class="bg-white dark:bg-gray-800 rounded-xl w-full max-w-md animate-fade-in">
            <div class="px-6 py-4 border-b border-primary-light dark:border-gray-700">
                <h3 class="text-lg font-semibold text-primary-dark dark:text-white">{{ __('Grade') }}</h3>
                <p class="text-sm text-primary-secondary" id="gradeStudentName"></p>
            </div>
            <form id="gradeForm" method="POST">
                @csrf
                <div class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-primary-dark dark:text-gray-300 mb-2">{{ __('Grade (0-100)') }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-primary-dark dark:text-gray-300 mb-2">{{ __('Feedback (Optional)') }}</label>
