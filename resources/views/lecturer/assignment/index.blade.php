<x-app-layout>
    ...
    <div class="card-saas p-6 mb-8 relative overflow-hidden">
        ...
        <div class="relative flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div class="flex items-center gap-5">
                ...
                <div>
                    <h2 class="text-2xl font-black text-primary-900 dark:text-white tracking-tight">{{ $class->course->course_name }}</h2>
                    <p class="text-primary-500 font-medium mt-1 flex items-center gap-2">
                        <span class="w-8 h-px bg-primary-200"></span>
                        {{ $class->course->course_code }} <span class="text-primary-300">•</span> {{ $class->course->credits }} {{ __('Credits') }}
                    </p>
                </div>
            </div>
            <button onclick="document.getElementById('createModal').classList.remove('hidden')" class="btn-primary-saas px-6 py-3 rounded-2xl text-sm font-black flex items-center gap-2 shadow-lg shadow-primary-600/20">
    ...
    <div class="space-y-6">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div class="flex-1">
                        <div class="flex flex-wrap items-center gap-3 mb-3">
                            <h3 class="text-lg font-black text-primary-900 dark:text-white group-hover:text-primary-primary transition-colors">{{ $assignment->title }}</h3>
                            @if(!$assignment->is_active)
                            <span class="px-2.5 py-1 text-[10px] font-black bg-primary-50 dark:bg-primary-900 text-primary-400 rounded-lg border border-primary-100 dark:border-primary-800 uppercase tracking-widest">{{ __('Inactive') }}</span>
                        ...
                        <p class="text-sm text-primary-500 font-medium mb-6 line-clamp-2">{{ $assignment->description }}</p>
                        @endif
                        <div class="flex flex-wrap items-center gap-6">
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    <p class="text-[10px] text-primary-400 font-black uppercase tracking-widest leading-none mb-1">{{ __('Deadline') }}</p>
                                    <p class="text-xs font-bold text-primary-700 dark:text-primary-300">{{ $assignment->deadline->format('d M Y, H:i') }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                    ...
                    <div class="flex items-center gap-3 lg:self-end">
                        <a href="{{ route('lecturers.assignment.show', [$class->id, $assignment->id]) }}" class="px-5 py-2.5 text-xs font-black bg-primary-900 dark:bg-primary-800 text-white rounded-xl hover:bg-primary-primary transition-all shadow-lg shadow-primary-900/10">
                            {{ __('View Submissions') }}
                        </a>
                        <form action="{{ route('lecturers.assignment.toggle', [$class->id, $assignment->id]) }}" method="POST">
                            @csrf
                            <button type="submit" class="p-2.5 text-primary-400 hover:text-primary-primary hover:bg-primary-50 dark:hover:bg-primary-800 rounded-xl transition-all border border-transparent hover:border-primary-100 dark:hover:border-primary-700" title="{{ $assignment->is_active ? __('Deactivate') : __('Activate') }}">
                                @if($assignment->is_active)
        ...
        <div class="card-saas p-12 text-center">
            ...
            <p class="text-primary-400 font-bold text-lg mb-2">{{ __('No assignments for this class yet.') }}</p>
            <p class="text-primary-300 text-sm mb-6">{{ __('Create your first assignment to start collecting submissions.') }}</p>
            <button onclick="document.getElementById('createModal').classList.remove('hidden')" class="btn-primary-saas px-6 py-3 rounded-2xl text-sm font-black shadow-lg shadow-primary-600/20">
            ...
            <div class="relative bg-white dark:bg-primary-900 rounded-[2rem] shadow-2xl w-full max-w-lg p-8 overflow-hidden transform transition-all animate-fade-in">
                ...
                <div class="relative">
                    ...
                    <form action="{{ route('lecturers.assignment.store', $class->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="space-y-5">
                            <div>
                                <label class="block text-[10px] font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('Assignment Title') }}</label>
                            ...
                            <div>
                                <label class="block text-[10px] font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('Deadline') }}</label>
                                <input type="datetime-local" name="deadline" class="input-saas w-full px-4 py-3 text-sm rounded-xl" required>
                            </div>
                            <div>
                                <label class="block text-[10px] font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('Question File (Optional)') }}</label>
                                <input type="file" name="file_assignment" class="input-saas w-full px-4 py-2 text-sm rounded-xl file:mr-4 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-black file:bg-primary-primary/10 file:text-primary-primary">
                            </div>
                            <div>
                                <label class="block text-[10px] font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('Allowed Extensions') }}</label>
                                <input type="text" name="allowed_extensions" class="input-saas w-full px-4 py-3 text-sm rounded-xl" value="pdf,doc,docx,zip,rar">
                                <p class="text-[10px] text-primary-400 mt-2 ml-1 italic">{{ __('Comma separated, no spaces') }}</p>
                            </div>
                        </div>
                        ...
                        <div class="mt-8 flex items-center justify-end gap-3">
                            ...
                            <button type="submit" class="btn-primary-saas px-8 py-2.5 rounded-xl text-sm font-black shadow-lg shadow-primary-600/20">{{ __('Save Assignment') }}</button>
                        </div>
                    </form>
                </div>
            </div>
