<x-app-layout>
    <div class="mb-10">
        <h1 class="text-3xl font-black tracking-tight text-siakad-900 dark:text-white">{{ __('Thesis Supervision') }}</h1>
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-2">
            <p class="text-siakad-500 font-medium flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div class="card-saas p-6 flex items-center gap-4 relative overflow-hidden group hover:-translate-y-1 transition-all duration-300">
            <div class="absolute top-0 right-0 -mr-8 -mt-8 w-24 h-24 bg-siakad-primary/5 rounded-full group-hover:scale-110 transition-transform duration-500"></div>
            <div class="w-14 h-14 rounded-2xl bg-siakad-50 dark:bg-siakad-900/50 flex items-center justify-center text-siakad-primary shadow-inner">
            ...
            <div>
                <p class="text-3xl font-black text-siakad-900 dark:text-white leading-none">{{ $thesisList->count() }}</p>
                <p class="text-[10px] text-siakad-400 font-black uppercase tracking-widest mt-2">{{ __('Total Supervised') }}</p>
            </div>
        </div>
        <div class="card-saas p-6 flex items-center gap-4 relative overflow-hidden group hover:-translate-y-1 transition-all duration-300">
            ...
            <div>
                <p class="text-3xl font-black text-siakad-900 dark:text-white leading-none">{{ $thesisList->where('status', 'supervision')->count() }}</p>
                <p class="text-[10px] text-siakad-400 font-black uppercase tracking-widest mt-2">{{ __('Active Supervision') }}</p>
            </div>
        </div>
        <div class="card-saas p-6 flex items-center gap-4 relative overflow-hidden group hover:-translate-y-1 transition-all duration-300">
            ...
            <div>
                <p class="text-3xl font-black text-siakad-900 dark:text-white leading-none">{{ $pendingSupervision }}</p>
                <p class="text-[10px] text-siakad-400 font-black uppercase tracking-widest mt-2">{{ __('Awaiting Feedback') }}</p>
            </div>
        </div>
        <div class="card-saas p-6 flex items-center gap-4 relative overflow-hidden group hover:-translate-y-1 transition-all duration-300">
            ...
            <div>
                <p class="text-3xl font-black text-siakad-900 dark:text-white leading-none">{{ $thesisList->where('status', 'completed')->count() }}</p>
                <p class="text-[10px] text-siakad-400 font-black uppercase tracking-widest mt-2">{{ __('Completed') }}</p>
            </div>
        </div>
    </div>
            ...
            <table class="w-full text-start border-collapse" id="thesisTable">
                <thead>
                    <tr class="bg-siakad-50/50 dark:bg-siakad-900/30 border-b border-siakad-100/50 dark:border-siakad-800/50">
                        ...
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start sortable-header cursor-pointer hover:text-siakad-primary transition-colors" data-sort="name" onclick="sortTable('name')">
                        ...
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-start sortable-header cursor-pointer hover:text-siakad-primary transition-colors" data-sort="title" onclick="sortTable('title')">
                            <div class="flex items-center gap-2">
                                {{ __('Title') }}
                                <svg class="w-3.5 h-3.5 opacity-30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4"></path>
                        ...
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center sortable-header cursor-pointer hover:text-siakad-primary transition-colors" data-sort="progress" onclick="sortTable('progress')">
                        ...
                        <th class="py-5 px-6 text-[10px] font-black text-siakad-400 uppercase tracking-widest text-center sortable-header cursor-pointer hover:text-siakad-primary transition-colors" data-sort="status" onclick="sortTable('status')">
                            <div class="flex items-center justify-center gap-2">
                                {{ __('Status') }}
                                <svg class="w-3.5 h-3.5 opacity-30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4"></path>
                ...
                <tbody id="tableBody" class="divide-y divide-siakad-50 dark:divide-siakad-800/50">
                    @forelse($thesisList as $index => $thesis)
                    <tr class="thesis-row hover:bg-siakad-50/30 dark:hover:bg-siakad-900/20 transition-colors group" 
                        ...
                        <td class="py-5 px-6 text-start">
                            <p class="text-sm font-black text-siakad-900 dark:text-white group-hover:text-siakad-primary transition-colors">{{ $thesis->student->user->name }}</p>
                            <p class="text-[10px] text-siakad-400 font-black uppercase tracking-widest mt-1">{{ $thesis->student->student_number }}</p>
                        </td>
                        <td class="py-5 px-6 text-start">
                            <p class="text-sm font-black text-siakad-700 dark:text-siakad-300 line-clamp-2" title="{{ $thesis->title }}">{{ $thesis->title }}</p>
                        </td>
                        <td class="py-5 px-6">
                            <div class="w-32 mx-auto">
                                <div class="flex items-center justify-between mb-1.5">
                                    <span class="text-[10px] font-black text-siakad-400 uppercase tracking-widest">{{ __('Completion') }}</span>
                                    <span class="text-[10px] font-black text-siakad-900 dark:text-white">{{ $thesis->progress_percent }}%</span>
                                </div>
                                <div class="h-1.5 bg-siakad-50 dark:bg-siakad-900/50 rounded-full overflow-hidden p-0.5 border border-siakad-100 dark:border-siakad-800">
                                    <div class="h-full bg-siakad-primary rounded-full shadow-[0_0_8px_rgba(37,99,235,0.4)] transition-all duration-500" style="width: {{ $thesis->progress_percent }}%"></div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="py-5 px-8 text-end">
                            <a href="{{ route('lecturers.thesis.show', $thesis) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-siakad-900 dark:bg-siakad-800 text-white text-xs font-black rounded-xl hover:bg-siakad-primary transition-all shadow-lg shadow-siakad-900/10">
                                {{ __('Details') }}
                    ...
                    <tr>
                        <td colspan="6" class="py-16 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-siakad-400 font-bold text-sm">{{ __('No students with thesis supervision yet.') }}</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
