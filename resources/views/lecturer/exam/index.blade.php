<x-app-layout>
    ...
    <div class="space-y-6">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div class="flex-1">
                        <div class="flex flex-wrap items-center gap-3 mb-3">
                            <h3 class="text-lg font-black text-primary-900 dark:text-white group-hover:text-primary-primary transition-colors">{{ $exam->title }}</h3>
                            @if($exam->isUpcoming())
                            ...
                            @else
                            <span class="px-2.5 py-1 text-[10px] font-black bg-primary-50 dark:bg-primary-900 text-primary-400 rounded-lg border border-primary-100 dark:border-primary-800 uppercase tracking-widest">{{ __('Past') }}</span>
                            @endif
                        ...
                        @if($exam->description)
                        <p class="text-sm text-primary-500 font-medium mb-6 line-clamp-2">{{ $exam->description }}</p>
                        @endif
                        <div class="flex flex-wrap items-center gap-6">
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    <p class="text-[10px] text-primary-400 font-black uppercase tracking-widest leading-none mb-1">{{ __('Exam Date') }}</p>
                                    <p class="text-xs font-bold text-primary-700 dark:text-primary-300">{{ $exam->formatted_exam_date }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                <div class="w-8 h-8 rounded-lg bg-primary-50 dark:bg-primary-900/50 flex items-center justify-center text-primary-400">
                                ...
                                <div>
                                    <p class="text-[10px] text-primary-400 font-black uppercase tracking-widest leading-none mb-1">{{ __('Duration') }}</p>
                                    <p class="text-xs font-bold text-primary-700 dark:text-primary-300">{{ $exam->duration }} {{ __('minutes') }}</p>
                                </div>
                            </div>
                            @if($exam->location)
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    <p class="text-[10px] text-primary-400 font-black uppercase tracking-widest leading-none mb-1">{{ __('Location') }}</p>
                                    <p class="text-xs font-bold text-primary-700 dark:text-primary-300">{{ $exam->location }}</p>
                                </div>
                            </div>
                            @endif
                            <div class="flex items-center gap-2">
        ...
        <div class="card-saas p-12 text-center">
            ...
            <p class="text-primary-400 font-bold text-lg mb-2">{{ __('No exams for this class yet.') }}</p>
            <p class="text-primary-300 text-sm mb-6">{{ __('Create your first exam to start scheduling.') }}</p>
            ...
            <div class="relative bg-white dark:bg-primary-900 rounded-[2rem] shadow-2xl w-full max-w-lg p-8 overflow-hidden transform transition-all animate-fade-in">
                ...
                <div class="relative">
                    ...
                    <form action="{{ route('lecturers.exam.store', $class->id) }}" method="POST">
                        ...
                        <div class="space-y-5">
                            <div>
                                <label class="block text-[10px] font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('Exam Title') }}</label>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-[10px] font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('Date & Time') }}</label>
                                    <input type="datetime-local" name="exam_date" class="input-saas w-full px-4 py-3 text-sm rounded-xl" required>
                                </div>
                                <div>
                                    <label class="block text-[10px] font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('Duration (min)') }}</label>
                                    <input type="number" name="duration" class="input-saas w-full px-4 py-3 text-sm rounded-xl" value="60" required min="1">
                                </div>
                            </div>
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-[10px] font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('Location') }}</label>
                                    <input type="text" name="location" class="input-saas w-full px-4 py-3 text-sm rounded-xl" placeholder="{{ __('e.g., Room 101') }}">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-black text-primary-400 uppercase tracking-widest mb-2 ml-1">{{ __('Max Score') }}</label>
                        ...
                        <div class="mt-8 flex items-center justify-end gap-3">
                            ...
                            <button type="submit" class="btn-primary-saas px-8 py-2.5 rounded-xl text-sm font-black shadow-lg shadow-primary-600/20">{{ __('Save Exam') }}</button>
                        </div>
                    </form>
                </div>
            </div>
